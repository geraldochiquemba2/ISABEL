...
const bgLight = "#FFF3E0";
const textDark = "#BF360C";

const inputCls =
  "w-full border border-[#e8d4c0] bg-white py-3 px-4 text-sm text-[#2a1a1a] placeholder:text-[#6B7280] outline-none focus:border-[#E65100] focus:ring-2 focus:ring-[#E65100]/10 transition-all rounded-xl";
...
export default function LoginDesporto() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "desporto-fitness" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "desporto-fitness" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-desporto"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "desporto-fitness", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "desporto-fitness" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-desporto"), 1000);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-[100dvh] text-[#2a1a1a]" style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: bgLight }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        <button
          onClick={() => window.location.href = "/desporto"}
          className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#E65100] transition-colors mb-12"
        >
          <ArrowLeft size={16} />
          Voltar
        </button>
        ...
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "19px", letterSpacing: "-.02em", color: accentColor }}>YESOLA<small style={{ display: "block", color: textDark, fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "8px", marginTop: "2px" }}>Desporto</small></span>
          </div>

          <div className="flex gap-6 mb-8 border-b border-[#e8d4c0]">
              ...
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                  mode === m
                    ...
                    : "border-transparent text-[#6B7280] hover:text-[#E65100]"
                }`}
              >
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Desporto & Fitness.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#E65100] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <CategoryMultiSelect options={DESPORTO_CATEGORIES} value={watch("categories") ?? []} onChange={(next)=>{setValue("categories", next, {shouldValidate:true}); setValue("category", next[0] || "", {shouldValidate:true});}} accent={accentColor} />
                <FieldError msg={regErr.categories?.message} />
              </div>

              <div>
                <label className={labelCls}>Província (Angola)</label>
                <LocationCombobox value={watch("province") ?? ""} options={ANGOLA_PROVINCES.map(p=>p.name)} onChange={(v)=>{setValue("province", v, {shouldValidate:true}); setValue("municipality","",{shouldValidate:true}); setValue("locality","",{shouldValidate:true});}} placeholder="Selecione a Província" accent={accentColor} />
                <FieldError msg={regErr.province?.message} />
              </div>

              <div>
                <label className={labelCls}>Município</label>
                <LocationCombobox value={watch("municipality") ?? ""} options={municipalities} onChange={(v)=>{setValue("municipality", v, {shouldValidate:true}); setValue("locality","",{shouldValidate:true});}} placeholder={selectedProvinceName ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!selectedProvinceName} accent={accentColor} />
                <FieldError msg={regErr.municipality?.message} />
              </div>

              <div>
                <label className={labelCls}>Localidade exacta (opcional)</label>
                <LocationCombobox value={watch("locality") ?? ""} options={getLocalities(watch("province") ?? "", watch("municipality") ?? "")} onChange={(v)=>setValue("locality", v, {shouldValidate:true})} placeholder="Ex: Benfica, Cazenga..." disabled={!watch("municipality")} accent={accentColor} />
              </div>
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="desporto-fitness" accentColor={accentColor} />
    </main>
  );
}

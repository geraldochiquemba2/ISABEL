...
export default function TurismoHome({ onBackToSelector }: { onBackToSelector?: () => void }) {
  ...
  const handleScopeSelect = (scope: Scope) => {
    ...
    navigate(`/explorar-turismo?` + params.toString());
    ...
    <div className="min-h-[100dvh] bg-[#e0f2f1] text-[#1a3a3a] pb-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section className="px-5 py-3">
        <button
          ...
          className="w-full flex items-center gap-4 bg-white rounded-2xl px-4 py-4 border border-[#b2dfdb] hover:border-[#00796B] transition-colors"
        >
          ...
          <div className="flex-1 text-left">
            <p className="text-[14px] font-semibold text-[#00796B]">
              {selectedProvince ? selectedProvince + (selectedMunicipality ? " · " + selectedMunicipality : "") : "Em todas as províncias de Angola"}
            </p>
            <p className="text-[11px] text-[#6B7280]">Encontre destinos e serviços turísticos perto de si.</p>
          </div>
      ...
      {showProvinceModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowProvinceModal(false)}>
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <WhereSearch onScope={handleScopeSelect} onClear={handleScopeClear} accent="#00796B" />
            </div>
                  ...
                  {filteredProvinces.length === 0 && (
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${
                        selectedProvince === province.name ? "bg-[#00796B] text-white font-medium" : "hover:bg-[#e0f2f1] text-[#171717]"
                      }`}
                    >
                      {province.name}
                    </button>
                  ...
                  <div className="space-y-1 max-h-[50vh] overflow-y-auto">
                    {filteredMunicipalities.length === 0 && (
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${
                          selectedMunicipality === municipality ? "bg-[#00796B] text-white font-medium" : "hover:bg-[#e0f2f1] text-[#171717]"
                        }`}
                      >
                        {municipality}
                      </button>
            ...
            {selectedProvince && (
              <button
                ...
                className="w-full mt-6 bg-[#00796B] text-white py-3 rounded-xl font-medium transition-colors"
              >

...
export default function LoginWeddings() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "weddings" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "weddings" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-weddings"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "weddings", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "weddings" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-weddings"), 1000);
    } catch (err: any) {

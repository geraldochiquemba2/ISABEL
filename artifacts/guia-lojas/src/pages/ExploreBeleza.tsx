...
const BELEZA_CATEGORIES = [
  ...
  { number: "04", title: "Rosto & Pele", intro: "Tratamentos faciais para uma pele radiante.", category: "rosto-pele", icon: Heart, items: ["Limpeza de Pele", "Tratamento de Acne", "Anti-aging & Rejuvenescimento", "Máscaras & Peelings"] },
  { number: "05", title: "Corpo & Spa", intro: "Relaxe e rejuvenesça o corpo e a mente.", category: "corpo-spa", icon: Flower2, items: ["Massagens & Drenagem", "Spa & Aromaterapia", "Tratamento Celulite", "Exfoliação & Hidratação"] },
  { number: "06", title: "Depilação", intro: "Pele suave e sem pelos com os melhores técnicos.", category: "depilacao", icon: Sparkles, items: ["Depilação Cera", "Depilação a Laser", "Sobrancelhas", "Design de Sobrancelhas"] },
];

function StoreCard({ store, productImages }: { store: any; productImages?: string[] }) {
  const fallbackImage = "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=400&h=300&fit=crop&auto=format&q=75";
...
export default function ExploreBeleza() {
  const [activeFilter, setActiveFilter] = useState<string | null>(() => {
    ...
    if (cat) {
      const group = BELEZA_CATEGORIES.find((g) => g.title.toLowerCase().includes(cat.toLowerCase()) || g.category.toLowerCase() === cat.toLowerCase());
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products", "beleza"],
    queryFn: async () => {
      const res = await fetch("/api/products?store_type=beleza");
      if (!res.ok) return [];
      return res.json();
    },
    staleTime: 60_000,
  ...
  return (
    <main className="min-h-[100dvh] bg-[#FBF7F2] text-[#292727]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="mx-auto max-w-[1380px] px-6 pt-28 pb-12 md:px-12">
        ...
        <div className="mb-6">
          <span className="text-xs uppercase tracking-[0.15em] text-[#697482] mr-2">Onde procuras?</span>
          <div className="mt-2 max-w-md">
            <WhereSearch onScope={setLocationScope} onClear={() => setLocationScope(null)} accent="#7A4549" />
          </div>
          {locationScope && (
            <button

...
const inputCls = "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#F57F17] focus:ring-2 focus:ring-[#F57F17]/10 transition-all rounded-xl";
...
const TRANSPORTES_CATEGORIES = [
  { number: "01", title: "Transporte de Passageiros", intro: "Serviços de transporte de pessoas.", items: ["Táxis", "Uber/Bolt", "Transporte Escolar", "Transfer Aeroportuário"], category: "passageiros" },
  { number: "02", title: "Transporte de Mercadorias", intro: "Logística e transporte de carga.", items: ["Transporte Rodoviário", "Transporte Fluvial", "Transporte Aéreo", "Carga Pesada"], category: "mercadorias" },
  { number: "03", title: "Aluguer de Viaturas", intro: "Aluguer de veículos para diversas necessidades.", items: ["Carros", "Motociclos", "Autocarros", "Maquinaria"], category: "aluguer" },
  { number: "04", title: "Logística & Armazenagem", intro: "Serviços de logística e armazenamento.", items: ["Armazéns", "Distribuição", "Gestão de Stock", "Cadeia de Frio"], category: "logistica" },
  { number: "05", title: "Mudanças & Carretos", intro: "Serviços de mudança e carreto.", items: ["Mudanças Residenciais", "Mudanças Comerciais", "Carretos", "Embalamento"], category: "mudancas" },
  { number: "06", title: "Reparação de Viaturas", intro: "Manutenção e reparação de veículos.", items: ["Oficinas Mecânicas", "Mecânica Geral", "Elétrica Automóvel", "Pneumáticos"], category: "reparacao-vc" },
  { number: "07", title: "Combustíveis & Postos", intro: "Distribuição de combustíveis.", items: ["Postos de Combustível", "Gasolina", "Diesel", "GPL"], category: "combustiveis" },
  { number: "08", title: "Seguros Automóveis", intro: "Seguros para veículos.", items: ["Seguro Obrigatório", "Seguro Todo o Risco", "Seguro de Frota", "Assistência em Viagem"], category: "seguros" },
  { number: "09", title: "Condução & Formação", intro: "Formação de motoristas e condutores.", items: ["Carta de Condução", "Cursos de Condução", "Formação Profissional", "Reciclagem"], category: "formacao-cond" },
  { number: "10", title: "Rastreamento & Tecnologia", intro: "Sistemas de rastreamento e gestão de frota.", items: ["Rastreamento GPS", "Gestão de Frota", "Telemetria", "Mobilidade"], category: "rastreamento" },
  { number: "11", title: "Transporte Especial", intro: "Transporte de passageiros especiais.", items: ["Transporte de Doentes", "Transporte Adaptado", "Transporte VIP", "Escolar"], category: "especial" },
  { number: "12", title: "Despachantes & Documentação", intro: "Serviços de despachante aduaneiro.", items: ["Despachantes", "Documentação", "Licenças", "Aduana"], category: "despachantes" },
];

export default function DashboardTransportes() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-transportes");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#fefbf0] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-xl font-bold tracking-tight text-[#1a3a1a]">Pedido de Conta Pendente</h1>
              <p className="text-sm text-[#6B7280]">A sua conta está em análise pela equipa de administração.</p>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#F57F17] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#E65100] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-transportes")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#fefbf0]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#fefbf0] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#F57F17] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#FFE082", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Transportes</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#F57F17] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#FFE082", marginTop: "1px" }}>Transportes</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="transportes-logistica" accentColor="#F57F17" />}
        ...
        {showChangePwd && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              <h3 className="font-['Playfair_Display'] text-xl text-[#1a3a1a] mb-4">Alterar senha</h3>
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowNewPwd(!showNewPwd)} className="absolute right-3 top-3 text-[#6B7280]">{showNewPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowConfirmPwd(!showConfirmPwd)} className="absolute right-3 top-3 text-[#6B7280]">{showConfirmPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#F57F17] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#E65100] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
                {!localUser?.mustChangePassword && <button onClick={() => setShowChangePwd(false)} className="flex-1 border border-[#d4e8d4] py-3 rounded-xl text-sm text-[#6B7280] hover:bg-gray-50 transition-colors">Cancelar</button>}
              </div>
            </motion.div>
          </motion.div>
        )}
...
function OverviewSection({ store }: { store: any }) {
  ...
  return (
    <div>
      <h2 className="font-['Playfair_Display'] text-3xl text-[#1a3a1a] mb-8">Visão Geral</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#d4e8d4] p-6">
            <div className="flex items-center gap-3 mb-3 text-[#6B7280]">{s.icon}<span className="text-xs uppercase tracking-wider">{s.label}</span></div>
            <p className="text-3xl font-semibold text-[#1a3a1a]">{s.value}</p>
          </div>
        ))}
        <div className="bg-white rounded-2xl border border-[#d4e8d4] p-5">
          <p className="text-[10px] text-[#6B7280] uppercase tracking-wider mb-1">Cliques WhatsApp</p>
          <p className="text-2xl font-bold text-[#1a3a1a]">{whatsappClicks}</p>
                </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#1a3a1a]">Minha Loja</h2>
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#F57F17] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#E65100] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl mb-6">
        <h3 className="font-['Playfair_Display'] text-lg text-[#1a3a1a]">Imagens</h3>
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d4e8d4] rounded-xl text-xs text-[#6B7280] hover:border-[#F57F17] hover:text-[#1a3a1a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#F57F17] hover:text-[#1a3a1a] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        <h3 className="font-['Playfair_Display'] text-lg text-[#1a3a1a]">Dados da loja</h3>
        ...
        <div>
          ...
          <CategoryMultiSelect options={getAreaCategories("transportes-logistica")} value={form.categories ?? []} onChange={(next) => { setForm((prev) => ({ ...prev, categories: next })); setDirty(true); }} max={4} accent="#F57F17" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <LocationCombobox label="Província" value={form.province} options={ANGOLA_PROVINCES.map((p) => p.name)} onChange={(v) => { handleChange("province", v); handleChange("municipality", ""); handleChange("locality", ""); }} placeholder="Selecione a Província" accent="#F57F17" />
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#F57F17" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#F57F17" />
        <div>
          <label className={labelCls}>Horários de funcionamento</label>
          <div className="space-y-3">
            {schedule.map((day, i) => (
              <div key={day.label} className="border border-[#d4e8d4] rounded-2xl p-4 bg-[#fafafa]">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-[#1a3a1a]">{day.label}</p>
                ...
                <div className={`grid grid-cols-2 gap-3 transition-all duration-200 ${day.closed ? "opacity-50 pointer-events-none" : ""}`}>
                  <div><p className="text-[10px] text-[#6B7280] mb-1.5 font-medium uppercase tracking-wide">Abertura</p><TimeSelect value={day.open} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, open: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                  <div><p className="text-[10px] text-[#6B7280] mb-1.5 font-medium uppercase tracking-wide">Fechamento</p><TimeSelect value={day.close} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, close: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                </div>
              </div>
            ))}
          </div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = TRANSPORTES_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#1a3a1a]">Serviços</h2>
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#F57F17] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#E65100] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              <div className="flex items-center justify-between">
                <h3 className="font-['Playfair_Display'] text-lg text-[#1a3a1a]">{editProduct ? "Editar serviço" : "Novo serviço"}</h3>
                <button onClick={() => { setShowForm(false); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="text-[#6B7280] hover:text-[#1a3a1a]"><X size={18} /></button>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#F57F17] hover:text-[#1a3a1a] cursor-pointer transition-colors">
                ...
                <p className="text-[10px] text-[#6B7280]">{productImages.length}/5 imagens</p>
              </div>
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Smartphone Samsung Galaxy S24" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {TRANSPORTES_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {TRANSPORTES_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#F57F17] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#E65100] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {TRANSPORTES_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {
          const groupProducts = getProductsForGroup(group.category);
          const isExpanded = selectedGroup === group.category;
          return (
            <div key={group.category} className="bg-white rounded-2xl border border-[#d4e8d4] overflow-hidden">
              <button onClick={() => setSelectedGroup(isExpanded ? null : group.category)} className="w-full flex items-center justify-between p-5 text-left hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-xs tracking-[0.2em] text-[#6B7280]">{group.number}</span>
                  <div>
                    <h4 className="text-sm font-semibold text-[#1a3a1a]">{group.title}</h4>
                    <p className="text-xs text-[#6B7280] mt-0.5">{groupProducts.length} serviço(s)</p>
                  </div>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-[#6B7280] transition-transform ${isExpanded ? "rotate-180" : ""}`}><path d="m6 9 6 6 6-6"/></svg>
              </button>
              {isExpanded && (
                <div className="border-t border-[#d4e8d4] p-5">
                  <p className="text-xs text-[#686e76] mb-4">{group.intro}</p>
                  <div className="mb-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6B7280] mb-2">Subcategorias</p>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6B7280] mb-2">Serviços desta categoria</p>
                    {groupProducts.length === 0 ? (
                      <p className="text-xs text-[#6B7280] text-center py-6 bg-[#fafafa] rounded-xl">Nenhum serviço nesta categoria.</p>
                    ) : (
                      <div className="space-y-2">
                        {groupProducts.map((p: any) => (
                          <div key={p.id} className="flex items-center gap-3 p-3 bg-[#fafafa] rounded-xl">
                            ...
                            <button onClick={() => { setEditProduct(p); setForm({ name: p.name, price: String(p.price || ""), currency: p.currency || "AOA", category: p.category || "", subcategory: p.subcategory || "", description: p.description || "" }); setProductImages(p.imageUrls && p.imageUrls.length > 0 ? p.imageUrls : (p.imageUrl ? [p.imageUrl] : [])); setShowForm(true); }} className="text-[#6B7280] hover:text-[#F57F17] transition-colors p-1"><Edit2 size={13} /></button>
                            <button onClick={() => { if (confirm("Eliminar este serviço?")) deleteMut.mutate(p.id); }} className="text-[#6B7280] hover:text-red-500 transition-colors p-1"><Trash2 size={13} /></button>
                          </div>
                        ))}
                      </div>
                    )}

...
const bgLight = "#FFFDE7";
const textDark = "#E65100";

const inputCls =
  "w-full border border-[#e8dcc0] bg-white py-3 px-4 text-sm text-[#2a2a1a] placeholder:text-[#6B7280] outline-none focus:border-[#F57F17] focus:ring-2 focus:ring-[#F57F17]/10 transition-all rounded-xl";
...
export default function LoginTransportes() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "transportes-logistica" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "transportes-logistica" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-transportes"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "transportes-logistica", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "transportes-logistica" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-transportes"), 1000);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-[100dvh] text-[#2a2a1a]" style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: bgLight }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        <button
          onClick={() => window.location.href = "/transportes"}
          className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#F57F17] transition-colors mb-12"
        >
          <ArrowLeft size={16} />
          Voltar
        </button>
        ...
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "19px", letterSpacing: "-.02em", color: accentColor }}>YESOLA<small style={{ display: "block", color: textDark, fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "8px", marginTop: "2px" }}>Transportes</small></span>
          </div>

          <div className="flex gap-6 mb-8 border-b border-[#e8dcc0]">
              ...
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                  mode === m
                    ...
                    : "border-transparent text-[#6B7280] hover:text-[#F57F17]"
                }`}
              >
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Transportes & Logística.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#F57F17] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <select className={`${inputCls} cursor-pointer`} {...regReg("category")}>
                  ...
                  {TRANSPORTES_CATEGORIES.map((cat) => (
                    <option key={cat} value={cat} className="bg-white text-[#2a2a1a]">
                      {cat}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {ANGOLA_PROVINCES.map((p) => (
                    <option key={p.name} value={p.name} className="bg-white text-[#2a2a1a]">{p.name}</option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {municipalities.map((m) => (
                    <option key={m} value={m} className="bg-white text-[#2a2a1a]">{m}</option>
                  ))}
                </select>
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="transportes-logistica" accentColor={accentColor} />
    </main>
  );
}

...
const bgLight = "#E0F2F1";
const textDark = "#004D40";

const inputCls =
  "w-full border border-[#c8ddd8] bg-white py-3 px-4 text-sm text-[#1a2a2a] placeholder:text-[#6B7280] outline-none focus:border-[#00796B] focus:ring-2 focus:ring-[#00796B]/10 transition-all rounded-xl";
...
export default function LoginTurismo() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "turismo-lazer" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "turismo-lazer" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-turismo"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "turismo-lazer", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "turismo-lazer" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-turismo"), 1000);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-[100dvh] text-[#1a2a2a]" style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: bgLight }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        <button
          onClick={() => window.location.href = "/turismo"}
          className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#00796B] transition-colors mb-12"
        >
          <ArrowLeft size={16} />
          Voltar
        </button>
        ...
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "19px", letterSpacing: "-.02em", color: accentColor }}>YESOLA<small style={{ display: "block", color: textDark, fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "8px", marginTop: "2px" }}>Turismo</small></span>
          </div>

          <div className="flex gap-6 mb-8 border-b border-[#c8ddd8]">
              ...
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                  mode === m
                    ...
                    : "border-transparent text-[#6B7280] hover:text-[#00796B]"
                }`}
              >
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Turismo & Lazer.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#00796B] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <select className={`${inputCls} cursor-pointer`} {...regReg("category")}>
                  ...
                  {TURISMO_CATEGORIES.map((cat) => (
                    <option key={cat} value={cat} className="bg-white text-[#1a2a2a]">
                      {cat}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {ANGOLA_PROVINCES.map((p) => (
                    <option key={p.name} value={p.name} className="bg-white text-[#1a2a2a]">{p.name}</option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {municipalities.map((m) => (
                    <option key={m} value={m} className="bg-white text-[#1a2a2a]">{m}</option>
                  ))}
                </select>
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="turismo-lazer" accentColor={accentColor} />
    </main>
  );
}

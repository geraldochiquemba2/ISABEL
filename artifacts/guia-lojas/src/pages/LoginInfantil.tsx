...
export default function LoginInfantil() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "infantil" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "infantil" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-infantil"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "infantil" });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "infantil" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-infantil"), 1000);
    } catch (err: any) {
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na Eliora Infantil & Maternidade.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#87909a] hover:text-[#8e44ad] underline underline-offset-2 transition-colors">

...
const ALIMENTACAO_CATEGORIES = [
  { number: "01", title: "Restaurantes", intro: "Gastronomia variada para todos os gostos.", items: ["Restaurantes Tradicionais", "Restaurantes Fast-Food", "Restaurantes Temáticos", "Self-Service"], category: "restaurantes" },
  { number: "02", title: "Cafés & Padarias", intro: "Aromas e sabores para começar bem o dia.", items: ["Cafetarias", "Padarias", "Confeitarias", "Pastelarias"], category: "cafes" },
  { number: "03", title: "Pizzarias & Hambúrguerias", intro: "A melhor pizza e hambúrguer da cidade.", items: ["Pizzarias", "Hambúrguerias", "Food Trucks", "Take Away"], category: "pizzarias" },
  { number: "04", title: "Churrascarias & Grelhados", intro: "Carnes grelhadas e churrascos irresistíveis.", items: ["Churrascarias", "Grelhados", "Espetadas", "Carne de Panela"], category: "churrascarias" },
  { number: "05", title: "Marisqueiras & Frutos do Mar", intro: "O melhor do mar na sua mesa.", items: ["Marisqueiras", "Peixarias", "Camarão & Lagosta", "Sushi & Sashimi"], category: "marisqueiras" },
  { number: "06", title: "Comida Rápida & Takeaway", intro: "Refeições rápidas e práticas.", items: ["Fast Food", "Comida Árabe", "Comida Chinesa", "Comida Indiana"], category: "comida-rapida" },
  { number: "07", title: "Sobremesas & Doces", intro: "Doces e sobremesas para adoçar a vida.", items: ["Pastelarias", "Chocolaterias", "Gelataria", "Doces Tradicionais"], category: "sobremesas" },
  { number: "08", title: "Bares & Pub", intro: "Bebidas e convívio em boa companhia.", items: ["Bares", "Pub", "Wine Bar", "Cocktail Bar"], category: "bares" },
  { number: "09", title: "Catering & Eventos", intro: "Serviços de catering para eventos especiais.", items: ["Catering Corporativo", "Catering de Casamentos", "Buffet", "Service de Coffe Break"], category: "catering" },
  { number: "10", title: "Supermercados & Mercearias", intro: "Produtos frescos e de qualidade.", items: ["Supermercados", "Mercearias", "Feiras Livres", "Produtos Orgânicos"], category: "supermercados" },
  { number: "11", title: "Bebidas & Distribuição", intro: "Distribuição de bebidas e bebidas variadas.", items: ["Cervejas", "Vinhos", "Destilados", "Sumos & Águas"], category: "bebidas" },
  { number: "12", title: "Confeitaria & Bolo Personalizado", intro: "Bolos e doces feitos à medida.", items: ["Bolos de Aniversário", "Bolos de Casamento", "Doces Finos", "Encomendas Especiais"], category: "confeitaria" },
];
...
export default function DashboardAlimentacao() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-alimentacao");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#fdf5f3] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#D84315] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#BF360C] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-alimentacao")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#fdf5f3]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#fdf5f3] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#D84315] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#FFAB91", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Alimentação</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#D84315] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#FFAB91", marginTop: "1px" }}>Alimentação</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="alimentacao-restauracao" accentColor="#D84315" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#D84315] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#BF360C] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#D84315] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#BF360C] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <CategoryMultiSelect options={getAreaCategories("alimentacao-restauracao")} value={(form as any).categories ?? []} onChange={(next) => handleChange("categories", next)} accent="#D84315" />
        </div>
        <LocationCombobox label="Província" value={form.province} options={ANGOLA_PROVINCES.map((p) => p.name)} onChange={(v) => { setForm((prev) => ({ ...prev, province: v, municipality: "", locality: "" })); setDirty(true); }} placeholder="Selecione a província" accent="#D84315" testId="select-store-province" />
        <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { setForm((prev) => ({ ...prev, municipality: v, locality: "" })); setDirty(true); }} placeholder={form.province ? "Selecione o município" : "Selecione a província primeiro"} disabled={!form.province} accent="#D84315" testId="select-store-municipality" />
        <LocationCombobox label="Localidade exacta" value={(form as any).locality || ""} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#D84315" testId="select-store-locality" />
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = ALIMENTACAO_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#D84315] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#BF360C] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#D84315] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Francesinha Clássica" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {ALIMENTACAO_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {ALIMENTACAO_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#D84315] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#BF360C] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {ALIMENTACAO_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

...
const inputCls = "w-full border border-[#A7B3C5] bg-white py-3 px-4 text-sm text-[#171717] placeholder:text-[#A7B3C5] outline-none focus:border-[#0B2D56] focus:ring-2 focus:ring-[#0B2D56]/10 transition-all rounded-xl";
const labelCls = "block text-[10px] font-semibold uppercase tracking-widest text-[#A7B3C5] mb-1.5";
...
const IMOVEIS_CATEGORIES = [
  { number: "01", title: "Hotéis & Resorts", intro: "Acolhimento e conforto de excelência para estadias inesquecíveis.", items: ["Hotéis Executivos", "Lodges e Resorts", "Aparthotéis & Suítes"], category: "hoteis" },
  { number: "02", title: "Alojamento & Estadias", intro: "Espaços pensados para quem busca conforto e praticidade.", items: ["Casas de Férias & Vilas", "Apartamentos Temporários (Short Stay)", "Quarto & Suíte de Passagem"], category: "alojamento" },
  { number: "03", title: "Imobiliária & Compras", intro: "Oportunidades reais para investir, comprar ou arrendar.", items: ["Venda de Casas & Apartamentos", "Terrenos & Lotes Urbanizados", "Arrendamento & Aluguer"], category: "imobiliaria" },
];
...
export default function DashboardImoveis() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-imoveis");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#F4EBD7] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-[#A7B3C5]">A sua conta está em análise pela equipa de administração.</p>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#0B2D56] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#0B2D56] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <a href="/login-imoveis" className="w-full border border-[#A7B3C5] text-[#A7B3C5] hover:bg-[#F4EBD7] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</a>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#F4EBD7]"><p className="text-sm text-[#A7B3C5]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#F4EBD7] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#0B2D56] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#D4AF37", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Imóveis</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#0B2D56] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#D4AF37", marginTop: "1px" }}>Imóveis</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="imoveis" accentColor="#0B2D56" />}
        ...
        {showChangePwd && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              <h3 className="font-['Playfair_Display'] text-xl text-[#171717] mb-4">Alterar senha</h3>
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowNewPwd(!showNewPwd)} className="absolute right-3 top-3 text-[#A7B3C5]">{showNewPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowConfirmPwd(!showConfirmPwd)} className="absolute right-3 top-3 text-[#A7B3C5]">{showConfirmPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#0B2D56] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#0B2D56] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
                {!localUser?.mustChangePassword && <button onClick={() => setShowChangePwd(false)} className="flex-1 border border-[#A7B3C5] py-3 rounded-xl text-sm text-[#A7B3C5] hover:bg-gray-50 transition-colors">Cancelar</button>}
              </div>
            </motion.div>
          </motion.div>
        )}
...
function OverviewSection({ store }: { store: any }) {
  const stats = [{ label: "Imóveis", value: store.products?.length || 0, icon: <Package size={18} /> }];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#A7B3C5] p-6">
            <div className="flex items-center gap-3 mb-3 text-[#A7B3C5]">{s.icon}<span className="text-xs uppercase tracking-wider">{s.label}</span></div>
            <p className="text-3xl font-semibold text-[#171717]">{s.value}</p>
          </div>
        ))}
        <div className="bg-white rounded-2xl border border-[#A7B3C5] p-5">
          <p className="text-[10px] text-[#A7B3C5] uppercase tracking-wider mb-1">Cliques WhatsApp</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#0B2D56] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0B2D56] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
        {saved && <span className="text-xs text-green-600 font-medium">Guardado!</span>}
      </div>

      <div className="bg-white rounded-2xl border border-[#A7B3C5] p-8 space-y-6 max-w-2xl mb-6">
        <h3 className="font-['Playfair_Display'] text-lg text-[#171717]">Imagens</h3>
        <div>
          <label className={labelCls}>Logo da loja</label>
          <div className="flex items-center gap-4">
            {store.logoUrl && <img src={store.logoUrl} alt="Logo" className="w-16 h-16 rounded-xl object-cover border border-[#A7B3C5]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#A7B3C5] rounded-xl text-xs text-[#A7B3C5] hover:border-[#0B2D56] hover:text-[#171717] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            {store.coverImage && <img src={store.coverImage} alt="Capa" className="w-32 h-20 rounded-xl object-cover border border-[#A7B3C5]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#A7B3C5] rounded-xl text-xs text-[#A7B3C5] hover:border-[#0B2D56] hover:text-[#171717] cursor-pointer transition-colors">
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            {(store.coverImages || []).map((img: string, i: number) => (
              <div key={i} className="relative group">
                <img src={img} alt={`Galeria ${i + 1}`} className="w-24 h-24 rounded-xl object-cover border border-[#A7B3C5]" />
            ...
            <label className="w-24 h-24 border border-dashed border-[#A7B3C5] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#A7B3C5] hover:border-[#0B2D56] hover:text-[#171717] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#A7B3C5] p-8 space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <CategoryMultiSelect options={getAreaCategories("imoveis")} value={form.categories ?? []} onChange={(next) => { setForm((prev) => ({ ...prev, categories: next })); setDirty(true); }} max={4} accent="#0B2D56" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <LocationCombobox label="Província" value={form.province} options={ANGOLA_PROVINCES.map((p) => p.name)} onChange={(v) => { handleChange("province", v); handleChange("municipality", ""); handleChange("locality", ""); }} placeholder="Selecione a Província" accent="#0B2D56" />
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#0B2D56" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#0B2D56" />
        <div>
          ...
          <div className="space-y-3">
            {schedule.map((day, i) => (
              <div key={day.label} className="border border-[#A7B3C5] rounded-2xl p-4 bg-[#FFFFFF]">
                ...
                <div className={`grid grid-cols-2 gap-3 transition-all duration-200 ${day.closed ? "opacity-50 pointer-events-none" : ""}`}>
                  <div><p className="text-[10px] text-[#A7B3C5] mb-1.5 font-medium uppercase tracking-wide">Abertura</p><TimeSelect value={day.open} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, open: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                  <div><p className="text-[10px] text-[#A7B3C5] mb-1.5 font-medium uppercase tracking-wide">Fechamento</p><TimeSelect value={day.close} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, close: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                </div>
              </div>
            ))}
          </div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = IMOVEIS_CATEGORIES.find((g) => g.title === form.category);
    ...
    onError: (error: Error) => { console.error("Erro ao guardar imóvel:", error.message); alert("Erro ao guardar: " + error.message); },
  });
  ...
  const handleProductImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (remaining <= 0) { alert("Máximo de 5 imagens por imóvel."); return; }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#171717]">Imóveis</h2>
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#0B2D56] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#0B2D56] transition-colors"><Plus size={15} /> Novo imóvel</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#A7B3C5] p-6 space-y-4 max-w-2xl">
              <div className="flex items-center justify-between">
                <h3 className="font-['Playfair_Display'] text-lg text-[#171717]">{editProduct ? "Editar imóvel" : "Novo imóvel"}</h3>
                <button onClick={() => { setShowForm(false); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="text-[#A7B3C5] hover:text-[#171717]"><X size={18} /></button>
              </div>
              <div>
                <label className={labelCls}>Imagens do imóvel (até 5)</label>
                <div className="flex flex-wrap gap-3 mb-3">
                  {productImages.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={img} alt={`Imagem ${i + 1}`} className="w-20 h-20 rounded-xl object-cover border border-[#A7B3C5]" />
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#A7B3C5] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#A7B3C5] hover:border-[#0B2D56] hover:text-[#171717] cursor-pointer transition-colors">
                ...
                <p className="text-[10px] text-[#A7B3C5]">{productImages.length}/5 imagens</p>
              </div>
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Apartamento T3" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {IMOVEIS_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {IMOVEIS_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#0B2D56] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0B2D56] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {IMOVEIS_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {
          const groupProducts = getProductsForGroup(group.category);
          const isExpanded = selectedGroup === group.category;
          return (
            <div key={group.category} className="bg-white rounded-2xl border border-[#A7B3C5] overflow-hidden">
              <button onClick={() => setSelectedGroup(isExpanded ? null : group.category)} className="w-full flex items-center justify-between p-5 text-left hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-xs tracking-[0.2em] text-[#A7B3C5]">{group.number}</span>
                  <div>
                    <h4 className="text-sm font-semibold text-[#171717]">{group.title}</h4>
                    <p className="text-xs text-[#A7B3C5] mt-0.5">{groupProducts.length} imóvel(is)</p>
                  </div>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-[#A7B3C5] transition-transform ${isExpanded ? "rotate-180" : ""}`}><path d="m6 9 6 6 6-6"/></svg>
              </button>
              {isExpanded && (
                <div className="border-t border-[#A7B3C5] p-5">
                  <p className="text-xs text-[#686e76] mb-4">{group.intro}</p>
                  <div className="mb-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#A7B3C5] mb-2">Subcategorias</p>
                    <div className="flex flex-wrap gap-2">{group.items.map((item) => <span key={item} className="px-3 py-1.5 bg-[#FAFAFA] text-xs text-[#565d66] rounded-full">{item}</span>)}</div>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#A7B3C5] mb-2">Imóveis desta categoria</p>
                    {groupProducts.length === 0 ? (
                      <p className="text-xs text-[#A7B3C5] text-center py-6 bg-[#FFFFFF] rounded-xl">Nenhum imóvel nesta categoria.</p>
                    ) : (
                      <div className="space-y-2">
                        {groupProducts.map((p: any) => (
                          <div key={p.id} className="flex items-center gap-3 p-3 bg-[#FFFFFF] rounded-xl">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-[10px] text-[#A7B3C5]">{p.subcategory || p.category} {p.price ? `· ${p.currency === "USD" ? "$" : p.currency === "EUR" ? "€" : p.currency === "GBP" ? "£" : "Kz"} ${p.price.toLocaleString("pt-AO")}` : ""}</p>
                            </div>
                            <button onClick={() => { setEditProduct(p); setForm({ name: p.name, price: String(p.price || ""), currency: p.currency || "AOA", category: p.category || "", subcategory: p.subcategory || "", description: p.description || "" }); setProductImages(p.imageUrls && p.imageUrls.length > 0 ? p.imageUrls : (p.imageUrl ? [p.imageUrl] : [])); setShowForm(true); }} className="text-[#A7B3C5] hover:text-[#0B2D56] transition-colors p-1"><Edit2 size={13} /></button>
                            <button onClick={() => { if (confirm("Eliminar este imóvel?")) deleteMut.mutate(p.id); }} className="text-[#A7B3C5] hover:text-red-500 transition-colors p-1"><Trash2 size={13} /></button>
                          </div>
                        ))}
                      </div>
                    )}

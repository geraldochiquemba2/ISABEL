...
const SERVICOS_CATEGORIES = [
  ...
  { number: "04", title: "Arquitectura & Engenharia", intro: "Projectos técnicos e soluções construtivas de excelência.", category: "arquitectura-engenharia", icon: Building2, items: ["Arquitectos", "Engenheiros Civis", "Engenheiros Electrotécnicos", "Engenheiros Mecânicos", "Desenho Técnico", "Projectos Arquitectónicos", "Fiscalização de Obras", "Consultoria Técnica"] },
...
function StoreCard({ store, productImages }: { store: any; productImages?: string[] }) {
  ...
  return (
    <div
      className="flex-shrink-0 w-48 rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-lg transition-shadow border border-[#C5CAE9] cursor-pointer hover:-translate-y-1"
      onClick={() => window.location.href = `/loja/${store.id}?from=servicos-profissionais`}
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-[#0D0D2B] truncate">{store.name}</h4>
        {store.description && (
          <p className="text-[10px] text-[#4A4A6B] mt-1 line-clamp-2">{store.description}</p>
        )}
      </div>
    </div>
  );
...
export default function ExploreServicosProfissionais() {
  const [activeFilter, setActiveFilter] = useState<string | null>(() => {
    ...
    if (cat) {
      const group = SERVICOS_CATEGORIES.find((g) => g.title.toLowerCase().includes(cat.toLowerCase()) || g.category.toLowerCase() === cat.toLowerCase());
  ...
  const { data: stores = [] } = useQuery({
    queryKey: ["stores", "servicos-profissionais"],
    queryFn: () => fetchStores({ storeType: "servicos-profissionais" }),
    staleTime: 60_000,
  });

  const { data: products = [] } = useQuery({
    queryKey: ["products", "servicos-profissionais"],
    queryFn: async () => {
      const res = await fetch("/api/products?store_type=servicos-profissionais");
      if (!res.ok) return [];
      return res.json();
    },
    staleTime: 60_000,
  ...
  const getStoresForGroup = (category: string) => {
    const group = SERVICOS_CATEGORIES.find((g) => g.category === category);
  ...
  return (
    <main className="min-h-[100dvh] bg-[#F5F5FF] text-[#0D0D2B]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#F5F5FF]/95 backdrop-blur-md border-b border-[#1A237E]/10">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-6 py-4 md:px-12">
          <button onClick={() => window.history.back()} className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#1A237E] transition-colors">
            <ArrowLeft size={16} /> Voltar
          </button>
          <span className="font-['Playfair_Display'] text-[19px] tracking-[-.02em] text-[#1A237E]">YESOLA<small className="block font-['DM_Sans'] text-[8px] uppercase tracking-[.23em] text-[#0D0D2B] mt-0.5">Serviços Profissionais</small></span>
          <a href="/explorar-servicos-prof" className="text-xs font-bold uppercase tracking-[0.14em] text-[#68727c] hover:text-[#1A237E] transition-colors hidden md:block">Explorar</a>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1380px] px-6 pt-28 pb-12 md:px-12">
        ...
        <div className="mb-16">
          <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#1A237E]">Explorar serviços</p>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          ...
          <button onClick={() => setActiveFilter(null)}
            className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
              activeFilter === null ? "bg-[#1A237E] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
            }`}>Todos</button>
          {SERVICOS_CATEGORIES.map((group) => (
            <button key={group.category} onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeFilter === group.category ? "bg-[#1A237E] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
              }`}>{group.number} {group.title.split(",")[0].split(" e ")[0]}</button>
          ))}
        </div>

        <div className="mb-6">
          <span className="text-xs uppercase tracking-[0.15em] text-[#4A4A6B] mr-2">Onde procuras?</span>
          <div className="mt-2 max-w-md">
            <WhereSearch onScope={setLocationScope} onClear={() => setLocationScope(null)} accent="#1A237E" />
          </div>
          {locationScope && (
            <button
        ...
        <div className="mb-6">
          <span className="text-xs uppercase tracking-[0.15em] text-[#4A4A6B] mr-2">Província:</span>
          <select
            value={activeProvince || ""}
            onChange={(e) => { setActiveProvince(e.target.value || null); setActiveMunicipality(null); }}
            className="mt-2 md:hidden w-full px-4 py-3 rounded-xl text-sm border border-[#C5CAE9] bg-white text-[#0D0D2B] outline-none"
          >
          ...
          <div className="hidden md:flex flex-wrap gap-3 mt-2">
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeProvince === null ? "bg-[#0D0D2B] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
              }`}>Todas</button>
            {provinces.map((province) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeProvince === province ? "bg-[#0D0D2B] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
                }`}>{province}</button>
            ))}
          </div>
        </div>
        ...
        {municipalities.length > 0 && (
          <div className="mb-12">
            <span className="text-xs uppercase tracking-[0.15em] text-[#4A4A6B] mr-2">Município:</span>
            <select
            ...
            <div className="hidden md:flex flex-wrap gap-3 mt-2">
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeMunicipality === null ? "bg-[#4A4A6B] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
                }`}>Todos</button>
              {municipalities.map((m) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                    activeMunicipality === m ? "bg-[#4A4A6B] text-white" : "bg-[#E8EAF6] text-[#68727c] hover:bg-[#C5CAE9]"
                  }`}>{m}</button>
              ))}
            </div>
          </div>
        ...
        <div>
          {filteredGroups.map((group, i) => {
            ...
            return (
              <article key={group.number} className={`group border-t border-[#1A237E]/20 py-8 md:py-12 ${i % 2 ? "md:ml-20" : ""}`}>
                <div className="grid gap-7 md:grid-cols-[100px_minmax(0,1fr)_minmax(260px,370px)] md:items-start">
                  <span className="font-['DM_Sans'] text-xs font-bold tracking-[0.2em] text-[#1A237E]">{group.number}</span>
                  <div>
                    <h3 className="max-w-xl font-['Playfair_Display'] text-3xl leading-[1.08] text-[#0D0D2B] md:text-[2.8rem]">{group.title}</h3>
                    <p className="mt-4 max-w-md text-sm leading-7 text-[#4A4A6B]">{group.intro}</p>
                    <ul className="mt-6 space-y-3 border-l border-[#1A237E]/30 pl-5 text-sm leading-5 text-[#4A4A6B]">
                      {group.items.map((item) => (
                        <li key={item}>
                          <div className="flex gap-3">
                            <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#1A237E]" />
                            <span>{item}</span>
                          </div>
                        </li>
                      ))}
                    ...
                    {groupStores.length > 0 && (
                      <button onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
                        className="mt-6 flex items-center gap-2 text-xs uppercase tracking-[0.15em] text-[#68727c] hover:text-[#1A237E] transition-colors">
                        Ver mais
                      </button>
                    )}
                  </div>
                  <div className="mt-4 md:mt-0">
                    <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.2em] text-[#4A4A6B] mb-3">Lojas/Serviços disponíveis</p>
                    {groupStores.length > 0 ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-[#1A237E]/25 p-6 text-center">
                        <p className="text-xs text-[#4A4A6B]">Em breve novas lojas</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="relative overflow-hidden border-t border-[#1A237E]/20 bg-[#1A237E] px-6 py-24 text-[#F5F5FF] md:px-12 md:py-32">
        <div className="relative mx-auto max-w-[1380px] md:flex md:items-end md:justify-between">
          <div>
            <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#C5CAE9]">O primeiro passo</p>
          ...
          <div className="mt-10 md:mt-0 md:w-80">
            <p className="text-sm leading-6 text-[#F5F5FF]/60">Conte-nos o que procura. A nossa equipa responde com tempo, atenção e cuidado.</p>
            <div className="mt-7">
              <a href="https://wa.me/244922001778?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20a%20YESOLA%20Serviços%20Profissionais." target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#0D0D2B] text-white text-sm font-medium rounded-full hover:bg-[#1A1A3E] transition-colors">
                Falar connosco
              </a>
            </div>
          </div>

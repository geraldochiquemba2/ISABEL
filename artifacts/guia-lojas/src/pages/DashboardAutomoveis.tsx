    ...
    <PageTransition>
      <div className="min-h-screen bg-[#f4f6f9] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="automoveis" accentColor="#0f1d32" />}
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-[#e8eaed] p-8 space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#0f1d32" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#0f1d32" />
        <div>

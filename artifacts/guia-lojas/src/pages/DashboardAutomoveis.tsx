...
import { AutomoveisAdminPanel } from "@/components/AutomoveisAdminPanel";
import { motion, AnimatePresence } from "framer-motion";

type Section = "overview" | "loja" | "produtos" | "contactos" | "admin";

const inputCls = "w-full border border-[#d1d4d8] bg-white py-3 px-4 text-sm text-[#30343a] placeholder:text-[#87909a] outline-none focus:border-[#0f1d32] focus:ring-2 focus:ring-[#0f1d32]/10 transition-all rounded-xl";
...
const AUTOMOVEIS_CATEGORIES = [
  ...
  { number: "04", title: "Peças & Acessórios", intro: "Peças originais e acessórios para todos os modelos.", items: ["Peças Originais", "Peças Genéricas", "Acessórios & Tuning"], category: "pecas-acessorios" },
  { number: "05", title: "Lavagem & Detailing", intro: "Lavagem profissional, polish e tratamento completo do seu veículo.", items: ["Lavagem Externa", "Detailing Completo", "Tratamento de Pele & Pintura"], category: "lavagem-detailing" },
  { number: "06", title: "Serviços de Transporte", intro: "Transporte de mercadorias e pessoas com segurança e pontualidade.", items: ["Transporte de Mercadorias", "Mudanças", "Transporte de Veículos"], category: "servicos-transporte" },
  { number: "07", title: "Assistência em Viagem", intro: "Assistência 24h para emergências rodoviárias em toda Angola.", items: ["Reboque 24h", "Assistência em Avaria", "Troca de Pneus"], category: "assistencia-viagem" },
  { number: "08", title: "Seguros Automóvel", intro: "Seguros completos para proteger o seu veículo e a sua família.", items: ["Seguro contra Terceiros", "Seguro Completo", "Seguro de Vidros"], category: "seguros-auto" },
  { number: "09", title: "Inspecção & Documentação", intro: "Inspecções periódicas e renovação de documentos do veículo.", items: ["Inspecção Periódica", "Renovação de Documentos", "Legalização de Veículos"], category: "insepcao-doc" },
  { number: "10", title: "Estacionamentos & Garagens", intro: "Estacionamentos seguros e garagens para guardar o seu veículo.", items: ["Estacionamento Diário", "Garagem Mensal", "Estacionamento Aeroporto"], category: "estacionamentos" },
];

export default function DashboardAutomoveis() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-automoveis");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f4f6f9] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="bg-amber-50/50 rounded-2xl border border-amber-100 p-4 text-xs text-amber-800 text-left space-y-2.5">
              ...
              <p>Assim que o administrador aprovar a sua solicitação, poderá aceder ao painel e gerenciar os seus serviços automóvel.</p>
            </div>
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#0f1d32] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#0a1525] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <a href="/login-automoveis" className="w-full border border-[#d1d4d8] text-[#87909a] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</a>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f4f6f9]"><p className="text-sm text-[#87909a]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f4f6f9] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#0f1d32] text-white px-4 py-3 flex items-center justify-between">
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#0f1d32] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#c9913a", marginTop: "1px" }}>Automóveis</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AutomoveisAdminPanel />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#0f1d32] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#0a1525] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
...
function AdminOverviewSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try {
      const all = await fetchAdminUsersFiltered("automoveis");
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#0f1d32] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0a1525] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#e8eaed] p-8 space-y-6 max-w-2xl mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d1d4d8] rounded-xl text-xs text-[#87909a] hover:border-[#0f1d32] hover:text-[#30343a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d1d4d8] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#87909a] hover:border-[#0f1d32] hover:text-[#30343a] cursor-pointer transition-colors">
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      const selectedCat = AUTOMOVEIS_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#30343a]">Serviços</h2>
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#0f1d32] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#0a1525] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#e8eaed] p-6 space-y-4 max-w-2xl">
              <div className="flex items-center justify-between">
                <h3 className="font-['Playfair_Display'] text-lg text-[#30343a]">{editProduct ? "Editar serviço" : "Novo serviço"}</h3>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d1d4d8] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#87909a] hover:border-[#0f1d32] hover:text-[#30343a] cursor-pointer transition-colors">
              ...
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Lavagem Completa" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {AUTOMOVEIS_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {AUTOMOVEIS_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#0f1d32] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0a1525] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {AUTOMOVEIS_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

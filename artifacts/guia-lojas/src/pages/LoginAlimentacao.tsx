...
const accentColor = "#D84315";
const bgLight = "#FFF3E0";
const textDark = "#8B2500";

const inputCls =
  "w-full border border-[#e8d4c8] bg-white py-3 px-4 text-sm text-[#2a1a1a] placeholder:text-[#6B7280] outline-none focus:border-[#D84315] focus:ring-2 focus:ring-[#D84315]/10 transition-all rounded-xl";
...
export default function LoginAlimentacao() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "alimentacao-restauracao" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "alimentacao-restauracao" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-alimentacao"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "alimentacao-restauracao", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "alimentacao-restauracao" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-alimentacao"), 1000);
    } catch (err: any) {
    ...
    <main className="min-h-[100dvh] text-[#2a1a1a]" style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: bgLight }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        <button
          ...
          className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#D84315] transition-colors mb-12"
        >
          <ArrowLeft size={16} />
          Voltar
        </button>
        ...
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "19px", letterSpacing: "-.02em", color: accentColor }}>YESOLA<small style={{ display: "block", color: textDark, fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "8px", marginTop: "2px" }}>Alimentação</small></span>
          </div>

          <div className="flex gap-6 mb-8 border-b border-[#e8d4c8]">
              ...
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                  mode === m
                    ...
                    : "border-transparent text-[#6B7280] hover:text-[#D84315]"
                }`}
              >
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Alimentação & Restauração.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#D84315] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <CategoryMultiSelect options={ALIMENTACAO_CATEGORIES} value={watch("categories") ?? []} onChange={(next)=>{setValue("categories", next, {shouldValidate:true}); setValue("category", next[0] || "", {shouldValidate:true});}} accent={accentColor} />
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="alimentacao-restauracao" accentColor={accentColor} />
    </main>
  );
}

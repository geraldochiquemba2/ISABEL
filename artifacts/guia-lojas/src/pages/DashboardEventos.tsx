...
const EVENTOS_CATEGORIES = [
  ...
  { number: "04", title: "Animação, Entretenimento e Atração de Pistas", intro: "Energia que faz a festa vibrar.", items: ["DJs, Bandas Musicais e Grupos Acústicos", "Animadores Infantis, Palhaços e Pinturas Faciais", "Aluguer de Insufláveis, Brinquedos e Trampolins"], category: "animacao" },
  { number: "05", title: "Staff Profissional e Segurança", intro: "Equipa de confiança para o seu evento.", items: ["Garçons, Barman, Copeiros e Cozinheiros para Festas", "Hostesses, Rececionistas e Promotores de Eventos", "Equipas de Segurança Privada e Controlo de Acessos", "Limpeza Antes, Durante e Pós-Evento"], category: "staff" },
  { number: "06", title: "Registo, Memória e Fotografia", intro: "Imortalize cada momento especial.", items: ["Fotógrafos de Festas e Eventos Sociais", "Fotógrafos Corporativos e de Galas", "Videomakers", "Cabines Fotográficas (Photobooth / 360º)"], category: "fotografia" },
];
...
export default function DashboardEventos() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-eventos");
  }, [localUser, setLoc]);
    ...
    <PageTransition>
      <div className="min-h-screen bg-[#fffcf9] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <div className="max-w-4xl"><h2 className="text-xl font-semibold text-[#3c2731] flex items-center gap-2"><ShieldAlert size={20} /> Administração</h2><p className="text-sm text-[#87909a] mt-2">Painel de administração em desenvolvimento.</p></div>}
...
function AdminOverviewSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try {
      const all = await fetchAdminUsersFiltered("eventos");
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="text-xl font-semibold text-[#3c2731] flex items-center gap-2"><KeyRound size={20} /> Pedidos de definir novas senhas</h2>
      ...
      <div className="space-y-3">
        {users.map((u) => (
          <div key={u.id} className="border border-[#d1d4d8] rounded-2xl p-4 bg-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            ...
            <button onClick={() => setResetId(u.id)} className="flex items-center gap-1 bg-[#8e5557] hover:bg-[#7a4a4c] text-white text-xs font-semibold px-4 py-2 rounded-full transition-colors"><KeyRound size={12} /> Reset Senha</button>
          </div>
        ))}
      ...
      {resetId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            <h3 className="text-sm font-semibold flex items-center gap-1.5"><KeyRound size={16} className="text-[#8e5557]" /> Redefinir Senha</h3>
            ...
            <div className="flex justify-end gap-2 text-xs">
              ...
              <button onClick={() => handleReset(resetId)} className="px-4 py-2 bg-[#8e5557] text-white rounded-full hover:bg-[#7a4a4c] transition-colors">Confirmar Reset</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        {EVENTOS_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

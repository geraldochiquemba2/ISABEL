...
const inputCls = "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#00796B] focus:ring-2 focus:ring-[#00796B]/10 transition-all rounded-xl";
...
const TURISMO_CATEGORIES = [
  ...
  { number: "04", title: "Turismo Cultural & Histórico", intro: "Descubra a riqueza cultural e histórica.", items: ["Monumentos & Sítios Históricos", "Museus & Galerias", "Turismo Religioso", "Património Cultural"], category: "cultural" },
  { number: "05", title: "Gastronomia & Enologia", intro: "Sabores autênticos e experiências gastronómicas.", items: ["Experiências Gastronómicas", "Degustação de Vinhos", "Cursos de Culinária", "Turismo Enológico"], category: "gastronomia" },
  { number: "06", title: "Transporte & Aluguer de Viaturas", intro: "Mobilidade e conforto nas suas deslocações.", items: ["Aluguer de Carros", "Aluguer de Motociclos", "Transporte Privado", "Transfer Aeroportuário"], category: "transporte" },
  { number: "07", title: "Desporto & Lazer", intro: "Atividades desportivas e de lazer.", items: ["Golfe", "Pesca Desportiva", "Surf & Windsurf", "Yoga & Meditação"], category: "desporto" },
  { number: "08", title: "Spa & Bem-Estar", intro: "Relaxe e recarregue as energias.", items: ["Spa & Massagens", "Tratamentos Corporais", "Sauna & Piscina", "Programas de Bem-Estar"], category: "spa" },
  { number: "09", title: "Eventos & Convenções", intro: "Espaços e serviços para eventos.", items: ["Centros de Convenções", "Salões de Eventos", "Casas de Festa", "Espaços ao Ar Livre"], category: "eventos" },
  { number: "10", title: "Ecoturismo & Sustentabilidade", intro: "Turismo responsável e ecológico.", items: ["Áreas Protegidas", "Turismo Rural", "Observação de Aves", "Turismo Sustentável"], category: "ecoturismo" },
  { number: "11", title: "Galeria & Fotografia", intro: "Registe e partilhe as suas experiências.", items: ["Fotografia Profissional", "Álbuns & Áreas", "Sessões Fotográficas", "Workshops de Fotografia"], category: "galeria" },
  { number: "12", title: "Guia Turístico & Intérpretes", intro: "Guia especializado e serviços de intérprete.", items: ["Guias Turísticos", "Intérpretes Multilíngues", "Roteiros Personalizados", "Turismo Acessível"], category: "guias" },
];

export default function DashboardTurismo() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-turismo");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f0faf9] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#00796B] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#004D40] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-turismo")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f0faf9]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f0faf9] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#00796B] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#80CBC4", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Turismo</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#00796B] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#80CBC4", marginTop: "1px" }}>Turismo</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="turismo-lazer" accentColor="#00796B" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#00796B] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#004D40] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#00796B] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#004D40] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d4e8d4] rounded-xl text-xs text-[#6B7280] hover:border-[#00796B] hover:text-[#1a3a1a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#00796B] hover:text-[#1a3a1a] cursor-pointer transition-colors">
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      const selectedCat = TURISMO_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#00796B] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#004D40] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#00796B] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Safari ao Parque Nacional" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {TURISMO_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {TURISMO_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#00796B] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#004D40] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {TURISMO_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

  ...
  return (
    <div className="min-h-[100dvh] bg-[#FAFAF8] text-[#17191A] pb-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .cat-item { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 12px 10px; background: white; border-radius: 14px; border: 1px solid #EEF3F4; min-width: 72px; cursor: pointer; transition: all 0.2s; }
        .cat-item:hover { border-color: #1E737B; background: #EEF3F4; }
        .trust-scroll { display: flex; gap: 8px; overflow-x: auto; scrollbar-width: none; }
        .trust-scroll::-webkit-scrollbar { display: none; }
        .trust-item { flex-shrink: 0; display: flex; align-items: center; gap: 8px; padding: 10px 16px; background: white; border-radius: 12px; border: 1px solid #EEF3F4; }
      `}</style>

      {/* Header */}
      <header className="sticky top-0 z-50 bg-[#FAFAF8]/95 backdrop-blur-md border-b border-[#EEF3F4]/60">
        <div className="flex items-center justify-between px-5 py-4">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] tracking-[0.25em] text-[#1E737B] font-semibold uppercase mt-0.5">Formações</span>
          </div>
        ...
        <div className="cat-scroll">
          {CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => navigate(`/explorar-formacoes?categoria=${cat.id}`)} className="cat-item">
              ...
              <span className="text-[10px] font-medium text-[#17191A] text-center leading-tight">{cat.name}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 py-4">
        <div className="relative rounded-2xl overflow-hidden" style={{ minHeight: "240px" }}>
          ...
          <div className="relative z-10 p-6 max-w-[60%]">
            <p className="text-[10px] tracking-[0.2em] text-[#1E737B] font-semibold uppercase">Aprender. Praticar. Avançar.</p>
            <h1 className="text-[28px] leading-[1.1] font-semibold mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>
              Invista no seu<br /><span className="text-[#1E737B]">futuro.</span>
            </h1>
            <p className="text-[12px] text-[#68757C] mt-3 leading-relaxed">Encontre os melhores cursos e formações para crescer pessoal e profissionalmente.</p>
            <button onClick={() => navigate("/explorar-formacoes")} className="mt-4 flex items-center gap-2 bg-[#1E737B] text-white text-[12px] font-medium px-4 py-2.5 rounded-full hover:bg-[#175A61] transition-colors">
      ...
      {showProvinceModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowProvinceModal(false)}>
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <WhereSearch onScope={handleScopeSelect} onClear={handleScopeClear} accent="#1E737B" />
            </div>
      ...
      {/* Counter */}
      <section className="px-5 py-3">
        <div className="flex items-center justify-center gap-2 bg-[#EEF3F4] rounded-2xl px-4 py-3 border border-[#EEF3F4]">
          <Heart size={16} className="text-[#1E737B]" />
          <span className="text-[13px] font-medium text-[#17191A]">Mais de <span className="text-[#1E737B] font-bold">+1.800</span> alunos a aprender e crescer todos os dias.</span>
        </div>
      </section>
        ...
        <div className="trust-scroll">
          {TRUST_BADGES.map((badge, i) => (
            <div key={i} className="trust-item">
              <span className="text-[#1E737B]">{badge.icon}</span>
              <span className="text-[11px] font-medium text-[#17191A]">{badge.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="text-center py-6 px-5">
        <p className="text-[11px] text-[#68757C]">YESOLA FORMAÇÕES · APRENDA. PRATIQUE. AVANÇE.</p>
      </div>
    </div>
  );
}

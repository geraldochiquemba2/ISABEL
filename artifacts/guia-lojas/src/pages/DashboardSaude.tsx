...
const SAUDE_CATEGORIES = [
  ...
  { number: "04", title: "Saúde Mental & Psicologia", intro: "Cuidar da mente é cuidar da vida.", items: ["Psicologia Clínica", "Psiquiatria", "Terapia de Casal & Familiar"], category: "saude-mental" },
  { number: "05", title: "Pediatria & Neonatologia", intro: "Cuidados especializados para os mais pequenos.", items: ["Pediatria Geral", "Neonatologia", "Desenvolvimento Infantil"], category: "pediatria" },
  { number: "06", title: "Terapias Alternativas & Bem-Estar", intro: "Equilíbrio e harmonia para o corpo e mente.", items: ["Acupunctura & Medicina Tradicional", "Aromaterapia & Fitoterapia", "Yoga & Meditação"], category: "terapias" },
  { number: "07", title: "Nutrição & Dietética", intro: "Alimentação saudável para uma vida melhor.", items: ["Nutrição Clínica", "Dietas Especiais", "Aconselhamento Nutricional"], category: "nutricao" },
  { number: "08", title: "Fisioterapia & Reabilitação", intro: "Recuperação e prevenção de lesões.", items: ["Fisioterapia Ortopédica", "Reabilitação Cardiopulmonar", "Pilates Terapêutico"], category: "fisioterapia" },
  { number: "09", title: "Análises Clínicas & Exames", intro: "Diagnóstico preciso e rápido.", items: ["Análises Clínicas Gerais", "Exames de Imagem", "Check-ups Completos"], category: "analises" },
  { number: "10", title: "Planos de Saúde & Seguros", intro: "Proteção e tranquilidade para si e a sua família.", items: ["Seguros de Saúde", "Planos Familiares", "Cobertura Internacional"], category: "planos" },
];
...
export default function DashboardSaude() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-saude");
  }, [localUser, setLoc]);
        ...
        <div className="min-h-screen bg-[#f0f7f0] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              <a href="/login-saude" className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</a>
            </div>
          </div>
        </div>
      </PageTransition>
      ...
      <div className="min-h-screen bg-[#f0f7f0] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#2E7D32] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#a5d6a7", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Saúde</small></span>
          </div>
        ...
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#2E7D32] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              ...
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#a5d6a7", marginTop: "1px" }}>Saúde</p>
...
function SaudeAdminPanel() {
  ...
  async function loadUsers() {
    setLoading(true);
    try {
      const all = await fetchAdminUsersFiltered("saude");
  ...
  if (loading) return <div className="text-center py-12 text-sm text-[#6B7280]">A carregar...</div>;

  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="text-xl font-semibold text-[#1a3a1a] flex items-center gap-2"><KeyRound size={20} /> Pedidos de definir novas senhas</h2>
        <p className="text-sm text-[#6B7280] mt-1">Utilizadores activos que podem precisar de redefinir senha.</p>
      </div>
      <div className="space-y-3">
        {users.map((u) => (
          <div key={u.id} className="border border-[#d4e8d4] rounded-2xl p-4 bg-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div className="flex items-start gap-3 flex-1">
              <div className="w-12 h-12 rounded-full overflow-hidden bg-[#f0f0f0] flex-shrink-0 flex items-center justify-center"><Phone size={18} className="text-[#6B7280]" /></div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#6B7280]">Loja: {u.storeName || "—"}</p>
              </div>
            </div>
        ...
        {users.length === 0 && <p className="text-center text-sm text-[#6B7280] py-8 border border-dashed rounded-2xl">Nenhum utilizador encontrado.</p>}
      </div>
      {resetId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d4e8d4] shadow-xl p-6 w-full max-w-sm space-y-4">
            <h3 className="text-sm font-semibold flex items-center gap-1.5"><KeyRound size={16} className="text-blue-500" /> Redefinir Senha</h3>
            <p className="text-xs text-[#6B7280]">A senha será redefinida para <strong>123456789</strong>.</p>
            <div className="flex justify-end gap-2 text-xs">
              <button onClick={() => setResetId(null)} className="px-4 py-2 border border-[#d4e8d4] rounded-full hover:bg-[#f0f0f0] transition-colors">Cancelar</button>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.province} onChange={(e) => { handleChange("province", e.target.value); handleChange("municipality", ""); }} className={`${inputCls} cursor-pointer`}>
              ...
              {ANGOLA_PROVINCES.map((p) => <option key={p.name} value={p.name} className="bg-white text-[#1a3a1a]">{p.name}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={form.municipality} onChange={(e) => handleChange("municipality", e.target.value)} className={`${inputCls} cursor-pointer disabled:opacity-50`} disabled={!form.province}>
              ...
              {(ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []).map((m) => <option key={m} value={m} className="bg-white text-[#1a3a1a]">{m}</option>)}
            </select>
          </div>
        </div>
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      const selectedCat = SAUDE_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Consulta de Cardiologia" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {SAUDE_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {SAUDE_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
      ...
      <div className="space-y-6">
        {SAUDE_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 max-w-2xl space-y-6">
        <div><label className={labelCls}>WhatsApp</label><a href={`https://wa.me/244${store.whatsapp || store.phone}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-[#2E7D32] hover:underline"><MessageCircle size={16} /> {store.whatsapp || store.phone}</a></div>

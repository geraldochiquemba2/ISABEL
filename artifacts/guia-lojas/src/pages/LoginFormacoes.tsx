...
const FORMACOES_CATEGORIES = [
  ...
  "Aulas Práticas, Artes, Música e Hobbies",
  "Saúde, Fitness e Treino Acompanhado",
...
export default function LoginFormacoes() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "formacoes" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "formacoes" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-formacoes"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "formacoes" });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "formacoes" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-formacoes"), 1000);
    } catch (err: any) {
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na Eliora Formações.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#87909a] hover:text-[#0c9894] underline underline-offset-2 transition-colors">

...
const AGRICULTURA_CATEGORIES = [
  ...
  { number: "04", title: "Sementes, Fertilizantes & Insumos", intro: "Materiais essenciais para o sucesso da produção.", category: "sementes-insumos", icon: Leaf, items: ["Sementes", "Fertilizantes", "Adubos", "Materiais agrícolas"] },
  { number: "05", title: "Máquinas & Equipamentos Agrícolas", intro: "Tecnologia e maquinário para modernizar a produção.", category: "maquinas-equipamentos", icon: Tractor, items: ["Tractores", "Máquinas", "Ferramentas", "Irrigação", "Equipamentos"] },
  { number: "06", title: "Avicultura", intro: "Criação de aves e produção de ovos.", category: "avicultura", icon: Scissors, items: ["Criação de aves", "Produção de ovos", "Pintos", "Produtos avícolas"] },
  { number: "07", title: "Pesca & Aquicultura", intro: "Exploração sustentável dos recursos aquáticos.", category: "pesca-aquicultura", icon: Fish, items: ["Pesca", "Piscicultura", "Criação de peixe", "Produção aquícola"] },
  { number: "08", title: "Produtos Agrícolas", intro: "Produtos frescos e transformados do campo.", category: "produtos-agricolas", icon: Sprout, items: ["Produtos frescos", "Transformados", "Produção local"] },
  { number: "09", title: "Serviços & Consultoria Agrícola", intro: "Apoio técnico e formação para o sector.", category: "servicos-consultoria", icon: Handshake, items: ["Assistência técnica", "Consultoria", "Formação agrícola", "Apoio à produção"] },
];

function StoreCard({ store, productImages }: { store: any; productImages?: string[] }) {
  const fallbackImage = "https://images.unsplash.com/photo-1500937386664-56d1dfef3854?w=400&h=300&fit=crop&auto=format&q=75";
  ...
  return (
    <div
      className="flex-shrink-0 w-48 rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-lg transition-shadow border border-[#C8E6C9] cursor-pointer hover:-translate-y-1"
      onClick={() => window.location.href = `/loja/${store.id}?from=agricultura`}
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-[#0A2E0D] truncate">{store.name}</h4>
        {store.description && (
          <p className="text-[10px] text-[#6B7280] mt-1 line-clamp-2">{store.description}</p>
        )}
      </div>
    </div>
  );
...
export default function ExploreAgricultura() {
  const [activeFilter, setActiveFilter] = useState<string | null>(() => {
    ...
    if (cat) {
      const group = AGRICULTURA_CATEGORIES.find((g) => g.title.toLowerCase().includes(cat.toLowerCase()) || g.category.toLowerCase() === cat.toLowerCase());
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products", "agricultura-agronegocio"],
    queryFn: async () => {
      const res = await fetch("/api/products?store_type=agricultura-agronegocio");
      if (!res.ok) return [];
      return res.json();
    },
    staleTime: 60_000,
  ...
  const getStoresForGroup = (category: string) => {
    const group = AGRICULTURA_CATEGORIES.find((g) => g.category === category);
  ...
  return (
    <main className="min-h-[100dvh] bg-[#F0F8F0] text-[#0A2E0D]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#F0F8F0]/95 backdrop-blur-md border-b border-[#2E7D32]/10">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-6 py-4 md:px-12">
          <button onClick={() => window.history.back()} className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#2E7D32] transition-colors">
            <ArrowLeft size={16} /> Voltar
          </button>
          <span className="font-['Playfair_Display'] text-[19px] tracking-[-.02em] text-[#2E7D32]">YESOLA<small className="block font-['DM_Sans'] text-[8px] uppercase tracking-[.23em] text-[#1B5E20] mt-0.5">Agricultura & Agronegócio</small></span>
          <a href="/explorar-agricultura" className="text-xs font-bold uppercase tracking-[0.14em] text-[#68727c] hover:text-[#2E7D32] transition-colors hidden md:block">Explorar</a>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1380px] px-6 pt-28 pb-12 md:px-12">
        ...
        <div className="mb-16">
          <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#2E7D32]">Explorar serviços</p>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          ...
          {AGRICULTURA_CATEGORIES.map((group) => (
            <button key={group.category} onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeFilter === group.category ? "bg-[#2E7D32] text-white" : "bg-[#e8f5e9] text-[#68727c] hover:bg-[#C8E6C9]"
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="mt-2 md:hidden w-full px-4 py-3 rounded-xl text-sm border border-[#C8E6C9] bg-white text-[#0A2E0D] outline-none"
          >
          ...
          <div className="hidden md:flex flex-wrap gap-3 mt-2">
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeProvince === null ? "bg-[#1B5E20] text-white" : "bg-[#e8f5e9] text-[#68727c] hover:bg-[#C8E6C9]"
              }`}>Todas</button>
            {provinces.map((province) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeProvince === province ? "bg-[#1B5E20] text-white" : "bg-[#e8f5e9] text-[#68727c] hover:bg-[#C8E6C9]"
                }`}>{province}</button>
            ))}
          </div>
        </div>
          ...
          <div className="mb-12">
            ...
            <div className="hidden md:flex flex-wrap gap-3 mt-2">
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeMunicipality === null ? "bg-[#6B7280] text-white" : "bg-[#e8f5e9] text-[#68727c] hover:bg-[#C8E6C9]"
                }`}>Todos</button>
              {municipalities.map((m) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                    activeMunicipality === m ? "bg-[#6B7280] text-white" : "bg-[#e8f5e9] text-[#68727c] hover:bg-[#C8E6C9]"
                  }`}>{m}</button>
              ))}
            </div>
          </div>
        ...
        <div>
          {filteredGroups.map((group, i) => {
            ...
            return (
              <article key={group.number} className={`group border-t border-[#2E7D32]/20 py-8 md:py-12 ${i % 2 ? "md:ml-20" : ""}`}>
                <div className="grid gap-7 md:grid-cols-[100px_minmax(0,1fr)_minmax(260px,370px)] md:items-start">
                  <span className="font-['DM_Sans'] text-xs font-bold tracking-[0.2em] text-[#1B5E20]">{group.number}</span>
                  <div>
                    <h3 className="max-w-xl font-['Playfair_Display'] text-3xl leading-[1.08] text-[#0A2E0D] md:text-[2.8rem]">{group.title}</h3>
                    <p className="mt-4 max-w-md text-sm leading-7 text-[#4A6B4D]">{group.intro}</p>
                    <ul className="mt-6 space-y-3 border-l border-[#1B5E20]/30 pl-5 text-sm leading-5 text-[#4A6B4D]">
                      {group.items.map((item) => (
                        <li key={item}>
                          <div className="flex gap-3">
                            <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#1B5E20]" />
                            <span>{item}</span>
                          </div>
                        </li>
                      ))}
                    ...
                    {groupStores.length > 0 && (
                      <button onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
                        className="mt-6 flex items-center gap-2 text-xs uppercase tracking-[0.15em] text-[#68727c] hover:text-[#2E7D32] transition-colors">
                        Ver mais
                      </button>
                    )}
                  </div>
                  <div className="mt-4 md:mt-0">
                    <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.2em] text-[#6B7280] mb-3">Lojas recentes</p>
                    {groupStores.length > 0 ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-[#2E7D32]/25 p-6 text-center">
      ...
      <section className="relative overflow-hidden border-t border-[#1B5E20]/20 bg-[#2E7D32] px-6 py-24 text-[#F0F8F0] md:px-12 md:py-32">
        <div className="relative mx-auto max-w-[1380px] md:flex md:items-end md:justify-between">
          <div>
            <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#a5d6a7]">O primeiro passo</p>
          ...
          <div className="mt-10 md:mt-0 md:w-80">
            <p className="text-sm leading-6 text-[#F0F8F0]/60">Conte-nos o que procura. A nossa equipa responde com tempo, atenção e cuidado.</p>
            <div className="mt-7">
              <a href="https://wa.me/244922001778?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20a%20YESOLA%20Agricultura%20%26%20Agronegócio." target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#1B5E20] text-white text-sm font-medium rounded-full hover:bg-[#145218] transition-colors">
                Falar connosco
              </a>
            </div>
          </div>

...
const inputCls = "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#1A237E] focus:ring-2 focus:ring-[#1A237E]/10 transition-all rounded-xl";
...
const SERVICOS_PROF_CATEGORIES = [
  ...
  { number: "04", title: "Marketing & Publicidade", intro: "Serviços de marketing e publicidade.", items: ["Marketing Digital", "Publicidade", "Relações Públicas", "Branding"], category: "marketing" },
  { number: "05", title: "Tecnologia & Informática", intro: "Soluções de TI e desenvolvimento.", items: ["Desenvolvimento Web", "Desenvolvimento Mobile", "Consultoria IT", "Suporte Técnico"], category: "tecnologia-sp" },
  { number: "06", title: "Recursos Humanos", intro: "Gestão de pessoal e RH.", items: ["Recrutamento", "Gestão de Pessoal", "Formação", "Avaliação de Desempenho"], category: "rh" },
  { number: "07", title: "Tradução & Interpretação", intro: "Serviços de tradução e interpretação.", items: ["Tradução Juramentada", "Interpretação", "Localização", "Subtitulação"], category: "traducao" },
  { number: "08", title: "Seguros & Consultoria", intro: "Serviços de seguros e consultoria.", items: ["Seguros de Vida", "Seguros Empresariais", "Consultoria de Risco", "Planeamento Financeiro"], category: "seguros-sp" },
  { number: "09", title: "Fotografia & Videografia", intro: "Serviços profissionais de imagem.", items: ["Fotografia Corporativa", "Videografia", "Edição", "Produção Audiovisual"], category: "fotografia-sp" },
  { number: "10", title: "Design & Comunicação", intro: "Serviços de design e comunicação visual.", items: ["Design Gráfico", "Design de Interiores", "Comunicação Visual", "Publicações"], category: "design-sp" },
  { number: "11", title: "Gestão de Projetos", intro: "Planejamento e gestão de projetos.", items: ["Gestão de Projetos", "Consultoria", "PMO", "Metodologias Ágeis"], category: "projetos" },
  { number: "12", title: "Mediação & Resolução de Conflitos", intro: "Serviços de mediação e arbitragem.", items: ["Mediação", "Arbitragem", "Negociação", "Conciliação"], category: "mediacao" },
];

export default function DashboardServiosProf() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-servicos-prof");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f3f4fb] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#1A237E] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#0D1547] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-servicos-prof")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f3f4fb]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f3f4fb] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#1A237E] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#7986CB", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Serviços Prof.</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#1A237E] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#7986CB", marginTop: "1px" }}>Serviços Prof.</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="servicos-profissionais" accentColor="#1A237E" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#1A237E] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#0D1547] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#1A237E] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D1547] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d4e8d4] rounded-xl text-xs text-[#6B7280] hover:border-[#1A237E] hover:text-[#1a3a1a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#1A237E] hover:text-[#1a3a1a] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#1A237E" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#1A237E" />
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = SERVICOS_PROF_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#1A237E] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D1547] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#1A237E] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {SERVICOS_PROF_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {SERVICOS_PROF_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#1A237E] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D1547] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {SERVICOS_PROF_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

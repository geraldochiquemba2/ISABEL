...
const inputCls =
  "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#2E7D32] focus:ring-2 focus:ring-[#2E7D32]/10 transition-all rounded-xl";
...
export default function LoginSaude() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "saude" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "saude" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-saude"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "saude", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "saude" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-saude"), 1000);
    } catch (err: any) {
    ...
    <main className="min-h-[100dvh] bg-[#f0f7f0] text-[#1a3a1a]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="flex gap-6 mb-8 border-b border-[#d4e8d4]">
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <button type="submit" className="w-full bg-[#2E7D32] text-white py-3 text-sm font-medium rounded-full hover:bg-[#1B5E20] transition-colors">
                Entrar
              </button>

              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Saúde & Bem-Estar.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#2E7D32] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <select className={`${inputCls} cursor-pointer`} {...regReg("category")}>
                  ...
                  {SAUDE_CATEGORIES.map((cat) => (
                    <option key={cat} value={cat} className="bg-white text-[#1a3a1a]">
                      {cat}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {ANGOLA_PROVINCES.map((p) => (
                    <option key={p.name} value={p.name} className="bg-white text-[#1a3a1a]">{p.name}</option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {municipalities.map((m) => (
                    <option key={m} value={m} className="bg-white text-[#1a3a1a]">{m}</option>
                  ))}
                </select>
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="saude" accentColor="#2E7D32" />
    </main>
  );
}

...
export default function ImoveisHome({ onBackToSelector }: { onBackToSelector?: () => void }) {
  ...
  const handleScopeSelect = (scope: Scope) => {
    ...
    navigate(`/explorar-imoveis?` + params.toString());
    ...
    <div className="min-h-[100dvh] bg-[#FAFAFA] text-[#171717] pb-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section className="px-5 py-3">
        <button
          ...
          className="w-full flex items-center gap-4 bg-white rounded-2xl px-4 py-4 border border-[#A7B3C5] hover:border-[#0B2D56] transition-colors"
        >
      ...
      {showProvinceModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowProvinceModal(false)}>
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <WhereSearch onScope={handleScopeSelect} onClear={handleScopeClear} accent="#0B2D56" />
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-semibold text-[#A7B3C5] uppercase tracking-wider mb-2">Província</h4>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${
                        selectedProvince === province.name
                          ? "bg-[#0B2D56] text-white font-medium"
                          : "hover:bg-[#F4EBD7] text-[#171717]"
                      }`}
                    >
                      {province.name}
                    </button>
              ...
              {selectedProvince && municipalities.length > 0 && (
                <div className="flex-1">
                  <h4 className="text-xs font-semibold text-[#A7B3C5] uppercase tracking-wider mb-2">Município</h4>
                  <div className="space-y-1 max-h-[50vh] overflow-y-auto">
      ...
      {(featured.length > 0 || fallbackFeatured.length > 0) && (
        <section className="px-5 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/explorar-imoveis")} className="text-[12px] font-medium text-[#0B2D56] flex items-center gap-1">
            ...
            {(featured.length > 0 ? featured : fallbackFeatured).map((store: any) => (
              <div key={store.id} className="flex-shrink-0 w-44 bg-white rounded-2xl overflow-hidden border border-[#A7B3C5] cursor-pointer" onClick={() => window.location.href = `/loja/${store.id}?from=imoveis`}>
                <div className="h-28 overflow-hidden">
                  <img src={store.coverImage || "https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=400&h=300&fit=crop&auto=format&q=80"} alt={store.name} className="w-full h-full object-cover" />
                </div>
                <div className="p-3">
                  <h4 className="text-[13px] font-semibold text-[#171717] truncate">{store.name}</h4>
                  <p className="text-[10px] text-[#A7B3C5] mt-0.5">{store.category}</p>
                  {store.province && (
                    <div className="flex items-center gap-1 mt-1">
                      <MapPin size={10} className="text-[#A7B3C5]" />
                      <span className="text-[10px] text-[#A7B3C5]">{store.province}</span>
                    </div>
                  )}
                </div>
              </div>

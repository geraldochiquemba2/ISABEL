...
const inputCls = "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#1565C0] focus:ring-2 focus:ring-[#1565C0]/10 transition-all rounded-xl";
...
const TECNOLOGIA_CATEGORIES = [
  { number: "01", title: "Smartphones & Tablets", intro: "Dispositivos móveis de última geração.", items: ["Smartphones Android", "Smartphones iPhone", "Tablets & iPads", "Acessórios para Smartphones"], category: "smartphones" },
  { number: "02", title: "Computadores & Laptops", intro: "Soluções de computação para trabalho e lazer.", items: ["Laptops", "Desktops", "Monitores", "Computadores Gaming"], category: "computadores" },
  { number: "03", title: "Áudio & Videotelefonia", intro: "Sons e imagens de alta qualidade.", items: ["Auriculares", "Altifalantes", "Soundbars", "Sistemas de Som"], category: "audio" },
  { number: "04", title: "Redes & Telecomunicações", intro: "Conectividade e infraestrutura de rede.", items: ["Roteadores", "Switches", "Cabeamento de Rede", "Access Points"], category: "redes" },
  { number: "05", title: "Impressão & Escritório", intro: "Equipamentos para produtividade no escritório.", items: ["Impressoras", "Scanners", "Cartuchos & Toner", "Papel & Acessórios"], category: "impressao" },
  { number: "06", title: "Componentes & Peças", intro: "Componentes para montagem e upgrade.", items: ["Processadores", "Placas-Mãe", "Memórias RAM", "Discos SSD & HD", "Fontes de Alimentação"], category: "componentes" },
  { number: "07", title: "Gaming & Entretenimento", intro: "Tudo para os apaixonados por jogos.", items: ["Consolas de Videogame", "Acessórios Gaming", "Teclados & Ratos Gaming", "Cadeiras Gaming"], category: "gaming" },
  { number: "08", title: "Casa Inteligente", intro: "Automatize a sua casa com tecnologia.", items: ["Asistentes Virtuais", "Iluminação Inteligente", "Segurança Inteligente", "Termostatos Inteligentes"], category: "casa-inteligente" },
  { number: "09", title: "Wearables & Gadgets", intro: "Tecnologia vestível e gadgets úteis.", items: ["Smartwatches", "Fitness Trackers", "Óculos Inteligentes", "Gadgets & Acessórios"], category: "wearables" },
  { number: "10", title: "Armazenamento & Memória", intro: "Espaço para todos os seus dados.", items: ["Discos Externos", "Pen Drives", "Cartões SD", "NAS & Serviços de Armazenamento"], category: "armazenamento" },
  { number: "11", title: "Software & Licenças", intro: "Soluções de software para diversas necessidades.", items: ["Sistemas Operativos", "Office & Produtividade", "Antivírus & Segurança", "Software de Design"], category: "software" },
  { number: "12", title: "Reparação & Assistência Técnica", intro: "Serviços de manutenção e reparação.", items: ["Reparação de Smartphones", "Reparação de Laptops", "Recuperação de Dados", "Manutenção Preventiva"], category: "reparacao" },
];

export default function DashboardTecnologia() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-tecnologia");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f0f4f8] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#1565C0] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#0D47A1] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-tecnologia")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f0f4f8]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f0f4f8] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#1565C0] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#90CAF9", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Tecnologia</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#1565C0] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#90CAF9", marginTop: "1px" }}>Tecnologia</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="tecnologia-electronicos" accentColor="#1565C0" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#1565C0] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#0D47A1] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#1565C0] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D47A1] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <CategoryMultiSelect options={getAreaCategories("tecnologia-electronicos")} value={(form as any).categories ?? []} onChange={(next) => handleChange("categories", next)} accent="#1565C0" />
        </div>
        <LocationCombobox label="Província" value={form.province} options={ANGOLA_PROVINCES.map((p) => p.name)} onChange={(v) => { setForm((prev) => ({ ...prev, province: v, municipality: "", locality: "" })); setDirty(true); }} placeholder="Selecione a província" accent="#1565C0" testId="select-store-province" />
        <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { setForm((prev) => ({ ...prev, municipality: v, locality: "" })); setDirty(true); }} placeholder={form.province ? "Selecione o município" : "Selecione a província primeiro"} disabled={!form.province} accent="#1565C0" testId="select-store-municipality" />
        <LocationCombobox label="Localidade exacta" value={(form as any).locality || ""} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#1565C0" testId="select-store-locality" />
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = TECNOLOGIA_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#1565C0] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D47A1] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#1565C0] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {TECNOLOGIA_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {TECNOLOGIA_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#1565C0] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#0D47A1] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {TECNOLOGIA_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

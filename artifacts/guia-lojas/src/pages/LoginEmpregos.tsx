...
const accentColor = "#4527A0";
const bgLight = "#EDE7F6";
const textDark = "#311B92";

const inputCls =
  "w-full border border-[#d4cee4] bg-white py-3 px-4 text-sm text-[#1a1a2a] placeholder:text-[#6B7280] outline-none focus:border-[#4527A0] focus:ring-2 focus:ring-[#4527A0]/10 transition-all rounded-xl";
...
export default function LoginEmpregos() {
  ...
  const onLoginSubmit = async (values: LoginValues) => {
    setError("");
    try {
      const res = await loginLojista({ ...values, storeType: "empregos-oportunidades" });
      const user = res.user;
      localStorage.setItem("guialocal_user", JSON.stringify({ ...user, storeType: "empregos-oportunidades" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-empregos"), 1000);
    } catch (err: any) {
  ...
  const onRegisterSubmit = async (values: RegisterValues) => {
    setError("");
    try {
      const res = await registerLojista({ ...values, storeType: "empregos-oportunidades", latitude: latitude || undefined, longitude: longitude || undefined });
      localStorage.setItem("guialocal_user", JSON.stringify({ ...res.user, storeType: "empregos-oportunidades" }));
      setSubmitted(true);
      setTimeout(() => setLoc("/dashboard-empregos"), 1000);
    } catch (err: any) {
    ...
    <main className="min-h-[100dvh] text-[#1a1a2a]" style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: bgLight }}>
      <div className="mx-auto max-w-[1380px] px-6 py-8 md:px-12">
        <button
          ...
          className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#4527A0] transition-colors mb-12"
        >
          <ArrowLeft size={16} />
          Voltar
        </button>
        ...
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "19px", letterSpacing: "-.02em", color: accentColor }}>YESOLA<small style={{ display: "block", color: textDark, fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "8px", marginTop: "2px" }}>Empregos</small></span>
          </div>

          <div className="flex gap-6 mb-8 border-b border-[#d4cee4]">
            {(["login", "register"] as const).map((m) => (
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                  mode === m
                    ? "border-[#4527A0] text-[#4527A0]"
                    : "border-transparent text-[#6B7280] hover:text-[#4527A0]"
                }`}
              >
          ...
          {submitted ? (
            ...
            <form onSubmit={loginSubmit(onLoginSubmit)} className="space-y-6">
              ...
              <div className="text-center pt-2">
                <a href={`https://wa.me/244922001778?text=${encodeURIComponent("Olá! Gostaria de redefinir a minha palavra-passe na YESOLA Empregos & Oportunidades.")}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6B7280] hover:text-[#4527A0] underline underline-offset-2 transition-colors">
            ...
            <form onSubmit={regSubmit(onRegisterSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <CategoryMultiSelect options={EMPREGOS_CATEGORIES} value={watch("categories") ?? []} onChange={(next)=>{setValue("categories", next, {shouldValidate:true}); setValue("category", next[0] || "", {shouldValidate:true});}} accent={accentColor} />
      ...
      <ForgotPasswordModal open={showForgotPwd} onClose={() => setShowForgotPwd(false)} storeType="empregos-oportunidades" accentColor={accentColor} />
    </main>
  );
}

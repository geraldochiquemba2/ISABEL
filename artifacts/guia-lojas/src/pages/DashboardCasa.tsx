...
const inputCls = "w-full border border-[#EDE8DE] bg-white py-3 px-4 text-sm text-[#2D2C2B] placeholder:text-[#87909a] outline-none focus:border-[#8B4513] focus:ring-2 focus:ring-[#8B4513]/10 transition-all rounded-xl";
...
export default function DashboardCasa() {
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#FFF8F0] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-xl font-bold tracking-tight text-[#2D2C2B]">Pedido de Conta Pendente</h1>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#8B4513] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#6B3410] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <a href="/login-casa" className="w-full border border-[#EDE8DE] text-[#87909a] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</a>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-[#FFF8F0] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#8B4513] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#FFF0E0", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Casa</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#8B4513] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#FFF0E0", marginTop: "1px" }}>Casa & Serviços</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="casa" accentColor="#8B4513" />}
        ...
        {showChangePwd && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              <h3 className="font-['Playfair_Display'] text-xl text-[#2D2C2B] mb-4">Alterar senha</h3>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#8B4513] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#6B3410] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
                {!localUser?.mustChangePassword && <button onClick={() => setShowChangePwd(false)} className="flex-1 border border-[#EDE8DE] py-3 rounded-xl text-sm text-[#87909a] hover:bg-gray-50 transition-colors">Cancelar</button>}
              </div>
            </motion.div>
          </motion.div>
        )}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#EDE8DE] p-6">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#2D2C2B]">Minha Loja</h2>
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#8B4513] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#6B3410] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
        {saved && <span className="text-xs text-green-600 font-medium">Guardado!</span>}
      </div>

      <div className="bg-white rounded-2xl border border-[#EDE8DE] p-8 space-y-6 max-w-2xl mb-6">
        <h3 className="font-['Playfair_Display'] text-lg text-[#2D2C2B]">Imagens</h3>
        <div>
          <label className={labelCls}>Logo da loja</label>
          <div className="flex items-center gap-4">
            {store.logoUrl && <img src={store.logoUrl} alt="Logo" className="w-16 h-16 rounded-xl object-cover border border-[#EDE8DE]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#EDE8DE] rounded-xl text-xs text-[#87909a] hover:border-[#8B4513] hover:text-[#2D2C2B] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            {store.coverImage && <img src={store.coverImage} alt="Capa" className="w-32 h-20 rounded-xl object-cover border border-[#EDE8DE]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#EDE8DE] rounded-xl text-xs text-[#87909a] hover:border-[#8B4513] hover:text-[#2D2C2B] cursor-pointer transition-colors">
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            {(store.coverImages || []).map((img: string, i: number) => (
              <div key={i} className="relative group">
                <img src={img} alt={`Galeria ${i + 1}`} className="w-24 h-24 rounded-xl object-cover border border-[#EDE8DE]" />
            ...
            <label className="w-24 h-24 border border-dashed border-[#EDE8DE] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#87909a] hover:border-[#8B4513] hover:text-[#2D2C2B] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#EDE8DE] p-8 space-y-6 max-w-2xl">
        <h3 className="font-['Playfair_Display'] text-lg text-[#2D2C2B]">Dados da loja</h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.province} onChange={(e) => { handleChange("province", e.target.value); handleChange("municipality", ""); }} className={`${inputCls} cursor-pointer`}>
              ...
              {ANGOLA_PROVINCES.map((p) => <option key={p.name} value={p.name} className="bg-white text-[#2D2C2B]">{p.name}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={form.municipality} onChange={(e) => handleChange("municipality", e.target.value)} className={`${inputCls} cursor-pointer disabled:opacity-50`} disabled={!form.province}>
              ...
              {(ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []).map((m) => <option key={m} value={m} className="bg-white text-[#2D2C2B]">{m}</option>)}
            </select>
          </div>
        </div>
        <div>
          ...
          <div className="space-y-3">
            {schedule.map((day, i) => (
              <div key={day.label} className="border border-[#EDE8DE] rounded-2xl p-4 bg-[#fafafa]">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-[#2D2C2B]">{day.label}</p>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#2D2C2B]">Serviços</h2>
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#8B4513] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#6B3410] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#EDE8DE] p-6 space-y-4 max-w-2xl">
              <div className="flex items-center justify-between">
                <h3 className="font-['Playfair_Display'] text-lg text-[#2D2C2B]">{editProduct ? "Editar serviço" : "Novo serviço"}</h3>
                <button onClick={() => { setShowForm(false); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="text-[#87909a] hover:text-[#2D2C2B]"><X size={18} /></button>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  {productImages.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={img} alt={`Imagem ${i + 1}`} className="w-20 h-20 rounded-xl object-cover border border-[#EDE8DE]" />
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#EDE8DE] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#87909a] hover:border-[#8B4513] hover:text-[#2D2C2B] cursor-pointer transition-colors">
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#8B4513] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#6B3410] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {CASA_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {
          ...
          return (
            <div key={group.category} className="bg-white rounded-2xl border border-[#EDE8DE] overflow-hidden">
              <button onClick={() => setSelectedGroup(isExpanded ? null : group.category)} className="w-full flex items-center justify-between p-5 text-left hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-xs tracking-[0.2em] text-[#87909a]">{group.number}</span>
                  <div>
                    <h4 className="text-sm font-semibold text-[#2D2C2B]">{group.title}</h4>
              ...
              {isExpanded && (
                <div className="border-t border-[#EDE8DE] p-5">
                        ...
                        {groupProducts.map((p: any) => (
                          <div key={p.id} className="flex items-center gap-3 p-3 bg-[#fafafa] rounded-xl">
                            ...
                            <div className="flex-1">
                              <h5 className="text-xs font-medium text-[#2D2C2B]">{p.name}</h5>
...
function ContactosSection({ store }: { store: any }) {
  return (
    <div>
      <h2 className="font-['Playfair_Display'] text-3xl text-[#2D2C2B] mb-8">Contactos</h2>
      <div className="bg-white rounded-2xl border border-[#EDE8DE] p-8 max-w-2xl space-y-6">
        <div><label className={labelCls}>WhatsApp</label><a href={`https://wa.me/244${store.whatsapp || store.phone}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-[#8B4513] hover:underline"><MessageCircle size={16} /> {store.whatsapp || store.phone}</a></div>
        <div><label className={labelCls}>Telefone</label><p className="text-sm text-[#2D2C2B]">{store.phone}</p></div>
        <div><label className={labelCls}>Endereço</label><p className="text-sm text-[#2D2C2B]">{store.address || "Não definido"}</p></div>
        <div>

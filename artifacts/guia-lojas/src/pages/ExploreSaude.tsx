...
function StoreCard({ store, productImages }: { store: any; productImages?: string[] }) {
  ...
  return (
    <div
      className="flex-shrink-0 w-48 rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-lg transition-shadow border border-[#d4e8d4] cursor-pointer hover:-translate-y-1"
      onClick={() => window.location.href = `/loja/${store.id}?from=saude`}
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-[#1a3a1a] truncate">{store.name}</h4>
        {store.description && (
...
export default function ExploreSaude() {
  const [activeFilter, setActiveFilter] = useState<string | null>(() => {
    ...
    if (cat) {
      const group = SAUDE_CATEGORIES.find((g) => g.title.toLowerCase().includes(cat.toLowerCase()) || g.category.toLowerCase() === cat.toLowerCase());
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products", "saude"],
    queryFn: async () => {
      const res = await fetch("/api/products?store_type=saude");
      if (!res.ok) return [];
      return res.json();
    },
    staleTime: 60_000,
  ...
  const getStoresForGroup = (category: string) => {
    const group = SAUDE_CATEGORIES.find((g) => g.category === category);
  ...
  return (
    <main className="min-h-[100dvh] bg-[#f0f7f0] text-[#1a3a1a]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#f0f7f0]/95 backdrop-blur-md border-b border-[#2E7D32]/10">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-6 py-4 md:px-12">
          ...
          <span className="font-['Playfair_Display'] text-[19px] tracking-[-.02em] text-[#2E7D32]">YESOLA<small className="block font-['DM_Sans'] text-[8px] uppercase tracking-[.23em] text-[#1B5E20] mt-0.5">Saúde & Bem-Estar</small></span>
          <a href="/explorar-saude" className="text-xs font-bold uppercase tracking-[0.14em] text-[#68727c] hover:text-[#2E7D32] transition-colors hidden md:block">Explorar</a>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1380px] px-6 pt-28 pb-12 md:px-12">
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="mt-2 md:hidden w-full px-4 py-3 rounded-xl text-sm border border-[#d4e8d4] bg-white text-[#1a3a1a] outline-none"
          >
            ...
            return (
              <article key={group.number} className={`group border-t border-[#2E7D32]/20 py-8 md:py-12 ${i % 2 ? "md:ml-20" : ""}`}>
                <div className="grid gap-7 md:grid-cols-[100px_minmax(0,1fr)_minmax(260px,370px)] md:items-start">
                  ...
                  <div>
                    <h3 className="max-w-xl font-['Playfair_Display'] text-3xl leading-[1.08] text-[#2E7D32] md:text-[2.8rem]">{group.title}</h3>
                    <p className="mt-4 max-w-md text-sm leading-7 text-[#2E7D32]/60">{group.intro}</p>
                    <ul className="mt-6 space-y-3 border-l border-[#1B5E20]/30 pl-5 text-sm leading-5 text-[#2E7D32]/70">
      ...
      <section className="relative overflow-hidden border-t border-[#1B5E20]/20 bg-[#2E7D32] px-6 py-24 text-[#f0f7f0] md:px-12 md:py-32">
        <div className="relative mx-auto max-w-[1380px] md:flex md:items-end md:justify-between">
          ...
          <div className="mt-10 md:mt-0 md:w-80">
            <p className="text-sm leading-6 text-[#f0f7f0]/60">Conte-nos o que procura. A nossa equipa responde com tempo, atenção e cuidado.</p>
            <div className="mt-7">
              <a href="https://wa.me/244922001778?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20a%20YESOLA%20Saúde%20%26%20Bem-Estar." target="_blank" rel="noopener noreferrer"

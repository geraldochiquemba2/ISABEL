...
import {
  ...
  FileText, Lightbulb, Keyboard, Building2, Languages, Palette, Crown,
  Search,
} from "lucide-react";
...
const CATEGORIES = [
  ...
  { id: "arquitectura-engenharia", name: "Arquitectura & Engenharia", icon: <Building2 size={28} className="text-[#1A237E]" /> },
...
const TRUST_BADGES = [
  ...
  { icon: <CreditCard size={18} />, label: "Contratação fácil" },
...
export default function ServicosProfHome({ onBackToSelector }: { onBackToSelector?: () => void }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#e8eaf6] text-[#1a1a3a] pb-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .cat-item { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 12px 10px; background: white; border-radius: 14px; border: 1px solid #c5cae9; min-width: 72px; cursor: pointer; transition: all 0.2s; }
        .cat-item:hover { border-color: #1A237E; background: #e8eaf6; }
        .trust-scroll { display: flex; gap: 8px; overflow-x: auto; scrollbar-width: none; }
        .trust-scroll::-webkit-scrollbar { display: none; }
        .trust-item { flex-shrink: 0; display: flex; align-items: center; gap: 8px; padding: 10px 16px; background: white; border-radius: 12px; border: 1px solid #c5cae9; }
      `}</style>

      {/* Header */}
      <header className="sticky top-0 z-50 bg-[#e8eaf6]/95 backdrop-blur-md border-b border-[#c5cae9]/60">
        <div className="flex items-center justify-between px-5 py-4">
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-1">{menuOpen ? <X size={22} /> : <Menu size={22} />}</button>
          <div className="flex flex-col items-center">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "26px", fontWeight: 600, color: "#1a1a3a" }}>YESOLA</span>
            <span className="text-[9px] tracking-[0.25em] text-[#1A237E] font-semibold uppercase mt-0.5">Serviços Profissionais</span>
          </div>
      ...
      <section className="px-5 pt-4 pb-2">
        <div className="cat-scroll">
          {CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => navigate(`/explorar-servicos-prof?categoria=${cat.id}`)} className="cat-item">
              <div className="w-10 h-10 flex items-center justify-center">{cat.icon}</div>
              <span className="text-[10px] font-medium text-[#1a1a3a] text-center leading-tight">{cat.name}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 py-4">
        <div className="relative rounded-2xl overflow-hidden" style={{ minHeight: "240px" }}>
          <img src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800&h=500&fit=crop&auto=format&q=80" alt="Serviços Profissionais" className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-white/90 via-white/60 to-transparent" />
          <div className="relative z-10 p-6 max-w-[60%]">
            <p className="text-[10px] tracking-[0.2em] text-[#1A237E] font-semibold uppercase">Excelência profissional.</p>
            <h1 className="text-[28px] leading-[1.1] font-semibold mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>
              <span className="text-[#1A237E]">Serviços</span> Profissionais.
            </h1>
            <p className="text-[12px] text-[#6B7280] mt-3 leading-relaxed">Encontre os melhores profissionais especializados para impulsionar o seu negócio.</p>
            <button onClick={() => navigate("/explorar-servicos-prof")} className="mt-4 flex items-center gap-2 bg-[#1A237E] text-white text-[12px] font-medium px-4 py-2.5 rounded-full hover:bg-[#0D1553] transition-colors">
      ...
      <section className="px-5 py-3">
        <button
          ...
          className="w-full flex items-center gap-4 bg-white rounded-2xl px-4 py-4 border border-[#c5cae9] hover:border-[#1A237E] transition-colors"
        >
          <div className="w-10 h-10 rounded-full bg-[#e8eaf6] flex items-center justify-center"><MapPin size={18} className="text-[#1A237E]" /></div>
          <div className="flex-1 text-left">
            <p className="text-[14px] font-semibold text-[#1A237E]">
              {selectedProvince ? selectedProvince + (selectedMunicipality ? " · " + selectedMunicipality : "") : "Em todas as províncias de Angola"}
            </p>
            <p className="text-[11px] text-[#6B7280]">Encontre serviços profissionais perto de si, onde estiver.</p>
          </div>
      ...
      {showProvinceModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowProvinceModal(false)}>
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <WhereSearch onScope={handleScopeSelect} onClear={handleScopeClear} accent="#1A237E" />
            </div>
      ...
      {(featured.length > 0 || fallbackFeatured.length > 0) && (
        <section className="px-5 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/explorar-servicos")} className="text-[12px] font-medium text-[#1E737B] flex items-center gap-1">
            ...
            {(featured.length > 0 ? featured : fallbackFeatured).map((store: any) => (
              <div key={store.id} className="flex-shrink-0 w-44 bg-white rounded-2xl overflow-hidden border border-[#EEF3F4] cursor-pointer" onClick={() => window.location.href = `/loja/${store.id}?from=servicos`}>
                <div className="h-28 overflow-hidden">
                  <img src={store.coverImage || "https://images.unsplash.com/photo-1521737711867-e3b97375f902?w=400&h=300&fit=crop&auto=format&q=80"} alt={store.name} className="w-full h-full object-cover" />
                </div>
                <div className="p-3">
      ...
      {isLoading ? (
        ...
        <StoreCategorySection categories={CATEGORIES} stores={stores} storeType="servicos-profissionais" exploreRoute="/explorar-servicos-prof" />
      )}

      {/* Counter */}
      <section className="px-5 py-3">
        <div className="flex items-center justify-center gap-2 bg-[#e8eaf6] rounded-2xl px-4 py-3 border border-[#c5cae9]">
          <Heart size={16} className="text-[#1A237E]" />
          <span className="text-[13px] font-medium text-[#1a1a3a]">Mais de <span className="text-[#1A237E] font-bold">+5.000</span> profissionais qualificados nas nossas plataformas.</span>
        </div>
      </section>
        ...
        <div className="trust-scroll">
          {TRUST_BADGES.map((badge, i) => (
            <div key={i} className="trust-item">
              ...
              <span className="text-[11px] font-medium text-[#1a1a3a]">{badge.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="text-center py-6 px-5">
        <p className="text-[11px] text-[#9CA3AF]">YESOLA SERVIÇOS · EXCELÊNCIA PROFISSIONAL.</p>
      </div>
    </div>
  );
}

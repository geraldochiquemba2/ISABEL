    ...
    <main className="min-h-[100dvh] overflow-x-hidden bg-[#fdf8f5] text-[#8e44ad]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="eliora-grain">
        ...
        <section id="contacto" className="relative overflow-hidden border-t border-[#e74c8c]/20 bg-[#8e44ad] px-6 py-14 text-[#fdf8f5] md:px-12 md:py-20">
          ...
          <div className="relative mx-auto max-w-[1380px] md:flex md:items-end md:justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.25em] text-[#e74c8c]">O primeiro passo</p><h2 className="mt-5 max-w-2xl font-['Playfair_Display'] text-5xl leading-[1.02] md:text-7xl">Vamos encontrar<br /><i>o melhor para o seu pequeno?</i></h2></div><div className="mt-10 md:mt-0 md:w-80"><p className="text-sm leading-6 text-[#fdf8f5]/60">Conte-nos o que procura. A nossa equipa responde com tempo, atenção e as melhores opções.</p><button onClick={() => { window.open("https://wa.me/244922001778?text=Ol%C3%A1%2C%20vim%20pela%20YESOLA%20Infantil%20%26%20Maternidade%20e%20gostaria%20de%20mais%20informa%C3%A7%C3%B5es.", "_blank"); setSent(true); }} className="mt-7 flex items-center gap-4 border-b border-[#e74c8c] pb-3 text-xs uppercase tracking-[.2em] text-[#fdf8f5]">{sent ? "Mensagem recebida" : "Falar com a equipa"} <ArrowRight size={15} /></button></div></div>
        </section>

        <footer className="mx-auto flex max-w-[1380px] flex-col gap-8 px-6 py-10 md:flex-row md:items-center md:justify-between md:px-12"><Monogram /><p className="text-xs text-[#8e44ad]/60">Amor e carinho, em Angola e além.</p><div className="flex items-center gap-5 text-[#8e44ad]/60"><a href="https://wa.me/244922001778?text=Ol%C3%A1%2C%20vim%20pela%20YESOLA%20Infantil%20%26%20Maternidade%20e%20gostaria%20de%20mais%20informa%C3%A7%C3%B5es." target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">WhatsApp</a><span className="font-mono text-[10px] tracking-[.2em]">© YESOLA</span></div></footer>
      </div>
    </main>
  );
}

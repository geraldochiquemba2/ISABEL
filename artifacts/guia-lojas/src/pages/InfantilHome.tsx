...
export default function InfantilHome({ onBackToSelector }: { onBackToSelector?: () => void }) {
  ...
  const handleScopeSelect = (scope: Scope) => {
    ...
    navigate(`/explorar-infantil?` + params.toString());
  ...
  return (
    <div className="min-h-[100dvh] bg-[#FFF7E6] text-[#171717] pb-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .cat-item { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 12px 10px; background: white; border-radius: 14px; border: 1px solid #E6DED3; min-width: 72px; cursor: pointer; transition: all 0.2s; }
        .cat-item:hover { border-color: #F7C948; background: #FFF7E6; }
        .trust-scroll { display: flex; gap: 8px; overflow-x: auto; scrollbar-width: none; }
        .trust-scroll::-webkit-scrollbar { display: none; }
        .trust-item { flex-shrink: 0; display: flex; align-items: center; gap: 8px; padding: 10px 16px; background: white; border-radius: 12px; border: 1px solid #E6DED3; }
      `}</style>

      {/* Header */}
      <header className="sticky top-0 z-50 bg-[#FFF7E6]/95 backdrop-blur-md border-b border-[#E6DED3]/60">
        <div className="flex items-center justify-between px-5 py-4">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] tracking-[0.25em] text-[#F7C948] font-semibold uppercase mt-0.5">Infantil</span>
          </div>

        </div>
        {menuOpen && (
          <div className="bg-[#FFF7E6] border-t border-[#E6DED3]/60 px-5 py-4 flex flex-col gap-3 text-sm font-medium">
      ...
      <section className="px-5 py-4">
        <div className="relative rounded-2xl overflow-hidden" style={{ minHeight: "240px" }}>
          ...
          <div className="relative z-10 p-6 max-w-[60%]">
            <p className="text-[10px] tracking-[0.2em] text-[#F7C948] font-semibold uppercase">Tudo para o seu pequeno.</p>
            <h1 className="text-[28px] leading-[1.1] font-semibold mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>
              Cuidar com<br /><span className="text-[#F7C948]">amor</span> e dedicação.
            </h1>
            <p className="text-[12px] text-[#E6DED3] mt-3 leading-relaxed">Creches, enxoval, brinquedos, alimentação e muito mais para o seu filho.</p>
            <button onClick={() => navigate("/explorar-infantil")} className="mt-4 flex items-center gap-2 bg-[#F7C948] text-white text-[12px] font-medium px-4 py-2.5 rounded-full hover:bg-[#C89B62] transition-colors">
      ...
      <section className="px-5 py-3">
        <button
          ...
          className="w-full flex items-center gap-4 bg-white rounded-2xl px-4 py-4 border border-[#E6DED3] hover:border-[#F7C948] transition-colors"
        >
          <div className="w-10 h-10 rounded-full bg-[#FFF7E6] flex items-center justify-center"><MapPin size={18} className="text-[#F7C948]" /></div>
          <div className="flex-1 text-left">
            <p className="text-[14px] font-semibold text-[#F7C948]">Escolha a sua província</p>
            <p className="text-[11px] text-[#E6DED3]">Encontre produtos infantis perto de si.</p>
          </div>
          <ChevronRight size={18} className="text-[#F7C948]" />
        </button>
      </section>
      ...
      {showProvinceModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowProvinceModal(false)}>
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <WhereSearch onScope={handleScopeSelect} onClear={handleScopeClear} accent="#F7C948" />
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-semibold text-[#E6DED3] uppercase tracking-wider mb-2">Província</h4>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${
                        selectedProvince === province.name
                          ? "bg-[#F7C948] text-white font-medium"
                          : "hover:bg-[#FFF7E6] text-[#171717]"
                      }`}
                    >
                      {province.name}
                    </button>
              ...
              {selectedProvince && municipalities.length > 0 && (
                <div className="flex-1">
                  <h4 className="text-xs font-semibold text-[#E6DED3] uppercase tracking-wider mb-2">Município</h4>
                  <div className="space-y-1 max-h-[50vh] overflow-y-auto">
            ...
            {selectedProvince && (
              <button
                ...
                className="w-full mt-6 bg-[#F7C948] text-white py-3 rounded-xl font-medium hover:bg-[#C89B62] transition-colors"
              >
      ...
      {(featured.length > 0 || fallbackFeatured.length > 0) && (
        <section className="px-5 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/explorar-infantil")} className="text-[12px] font-medium text-[#F7C948] flex items-center gap-1">
            ...
            {(featured.length > 0 ? featured : fallbackFeatured).map((store: any) => (
              <div key={store.id} className="flex-shrink-0 w-44 bg-white rounded-2xl overflow-hidden border border-[#E6DED3] cursor-pointer" onClick={() => window.location.href = `/loja/${store.id}?from=infantil`}>
                ...
                <div className="p-3">
                  ...
                  <p className="text-[10px] text-[#E6DED3] mt-0.5">{store.category}</p>
                  {store.province && (
                    <div className="flex items-center gap-1 mt-1">
                      <MapPin size={10} className="text-[#E6DED3]" />
                      <span className="text-[10px] text-[#E6DED3]">{store.province}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* Counter */}
      <section className="px-5 py-3">
        <div className="flex items-center justify-center gap-2 bg-[#FFF7E6] rounded-2xl px-4 py-3 border border-[#E6DED3]">
          <Heart size={16} className="text-[#F7C948]" />
          <span className="text-[13px] font-medium text-[#171717]">Mais de <span className="text-[#F7C948] font-bold">+3.500</span> famílias satisfeitas com os nossos serviços infantis.</span>
        </div>
      </section>
        ...
        <div className="trust-scroll">
          {TRUST_BADGES.map((badge, i) => (
            <div key={i} className="trust-item">
              <span className="text-[#F7C948]">{badge.icon}</span>
      ...
      <div className="text-center py-6 px-5">
        <p className="text-[11px] text-[#E6DED3]">YESOLA INFANTIL · TUDO PARA O SEU PEQUENO COM AMOR.</p>
      </div>
    </div>
  );
}

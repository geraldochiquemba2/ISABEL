...
const inputCls = "w-full border border-[#EAD9D5] bg-white py-3 px-4 text-sm text-[#292727] placeholder:text-[#697482] outline-none focus:border-[#7A4549] focus:ring-2 focus:ring-[#7A4549]/10 transition-all rounded-xl";
const labelCls = "block text-[10px] font-semibold uppercase tracking-widest text-[#697482] mb-1.5";
...
const BELEZA_CATEGORIES = [
  ...
  { number: "04", title: "Rosto & Pele", intro: "Tratamentos faciais para uma pele radiante.", items: ["Limpeza de Pele", "Tratamento de Acne", "Anti-aging & Rejuvenescimento", "Máscaras & Peelings"], category: "rosto-pele" },
  { number: "05", title: "Corpo & Spa", intro: "Relaxe e rejuvenesça o corpo e a mente.", items: ["Massagens & Drenagem", "Spa & Aromaterapia", "Tratamento Celulite", "Exfoliação & Hidratação"], category: "corpo-spa" },
  { number: "06", title: "Depilação", intro: "Pele suave e sem pelos com os melhores técnicos.", items: ["Depilação Cera", "Depilação a Laser", "Sobrancelhas", "Design de Sobrancelhas"], category: "depilacao" },
];

export default function DashboardBeleza() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-beleza");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#FBF7F2] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-xl font-bold tracking-tight text-[#292727]">Pedido de Conta Pendente</h1>
              <p className="text-sm text-[#697482]">A sua conta está em análise pela equipa de administração.</p>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#7A4549] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#5A3335] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <a href="/login-beleza" className="w-full border border-[#EAD9D5] text-[#697482] hover:bg-[#F0EBE6] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</a>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#FBF7F2]"><p className="text-sm text-[#697482]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#FBF7F2] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#7A4549] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#EAD9D5", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Beleza</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#7A4549] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#EAD9D5", marginTop: "1px" }}>Beleza</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="beleza" accentColor="#7A4549" />}
        ...
        {showChangePwd && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              <h3 className="font-['Playfair_Display'] text-xl text-[#292727] mb-4">Alterar senha</h3>
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowNewPwd(!showNewPwd)} className="absolute right-3 top-3 text-[#697482]">{showNewPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowConfirmPwd(!showConfirmPwd)} className="absolute right-3 top-3 text-[#697482]">{showConfirmPwd ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                </div>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#7A4549] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#5A3335] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
                {!localUser?.mustChangePassword && <button onClick={() => setShowChangePwd(false)} className="flex-1 border border-[#EAD9D5] py-3 rounded-xl text-sm text-[#697482] hover:bg-gray-50 transition-colors">Cancelar</button>}
              </div>
            </motion.div>
          </motion.div>
        )}
...
function OverviewSection({ store }: { store: any }) {
  ...
  return (
    <div>
      <h2 className="font-['Playfair_Display'] text-3xl text-[#292727] mb-8">Visão Geral</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#EAD9D5] p-6">
            <div className="flex items-center gap-3 mb-3 text-[#697482]">{s.icon}<span className="text-xs uppercase tracking-wider">{s.label}</span></div>
            <p className="text-3xl font-semibold text-[#292727]">{s.value}</p>
          </div>
        ))}
        <div className="bg-white rounded-2xl border border-[#EAD9D5] p-5">
          <p className="text-[10px] text-[#697482] uppercase tracking-wider mb-1">Cliques WhatsApp</p>
          <p className="text-2xl font-bold text-[#292727]">{whatsappClicks}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#292727]">Minha Loja</h2>
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#7A4549] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#5A3335] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
        {saved && <span className="text-xs text-green-600 font-medium">Guardado!</span>}
      </div>

      <div className="bg-white rounded-2xl border border-[#EAD9D5] p-8 space-y-6 max-w-2xl mb-6">
        <h3 className="font-['Playfair_Display'] text-lg text-[#292727]">Imagens</h3>
        <div>
          <label className={labelCls}>Logo da loja</label>
          <div className="flex items-center gap-4">
            {store.logoUrl && <img src={store.logoUrl} alt="Logo" className="w-16 h-16 rounded-xl object-cover border border-[#EAD9D5]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#EAD9D5] rounded-xl text-xs text-[#697482] hover:border-[#7A4549] hover:text-[#292727] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            {store.coverImage && <img src={store.coverImage} alt="Capa" className="w-32 h-20 rounded-xl object-cover border border-[#EAD9D5]" />}
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#EAD9D5] rounded-xl text-xs text-[#697482] hover:border-[#7A4549] hover:text-[#292727] cursor-pointer transition-colors">
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            {(store.coverImages || []).map((img: string, i: number) => (
              <div key={i} className="relative group">
                <img src={img} alt={`Galeria ${i + 1}`} className="w-24 h-24 rounded-xl object-cover border border-[#EAD9D5]" />
            ...
            <label className="w-24 h-24 border border-dashed border-[#EAD9D5] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#697482] hover:border-[#7A4549] hover:text-[#292727] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#EAD9D5] p-8 space-y-6 max-w-2xl">
        <h3 className="font-['Playfair_Display'] text-lg text-[#292727]">Dados da loja</h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.province} onChange={(e) => { handleChange("province", e.target.value); handleChange("municipality", ""); }} className={`${inputCls} cursor-pointer`}>
              ...
              {ANGOLA_PROVINCES.map((p) => <option key={p.name} value={p.name} className="bg-white text-[#292727]">{p.name}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={form.municipality} onChange={(e) => handleChange("municipality", e.target.value)} className={`${inputCls} cursor-pointer disabled:opacity-50`} disabled={!form.province}>
              ...
              {(ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []).map((m) => <option key={m} value={m} className="bg-white text-[#292727]">{m}</option>)}
            </select>
          </div>
        </div>
        <div>
          ...
          <div className="space-y-3">
            {schedule.map((day, i) => (
              <div key={day.label} className="border border-[#EAD9D5] rounded-2xl p-4 bg-[#fafafa]">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-[#292727]">{day.label}</p>
                ...
                <div className={`grid grid-cols-2 gap-3 transition-all duration-200 ${day.closed ? "opacity-50 pointer-events-none" : ""}`}>
                  <div><p className="text-[10px] text-[#697482] mb-1.5 font-medium uppercase tracking-wide">Abertura</p><TimeSelect value={day.open} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, open: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                  <div><p className="text-[10px] text-[#697482] mb-1.5 font-medium uppercase tracking-wide">Fechamento</p><TimeSelect value={day.close} onChange={(v) => { setSchedule((prev) => prev.map((d, idx) => idx === i ? { ...d, close: v } : d)); setDirty(true); }} disabled={day.closed} /></div>
                </div>
              </div>
            ))}
          </div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      const selectedCat = BELEZA_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-['Playfair_Display'] text-3xl text-[#292727]">Serviços</h2>
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#7A4549] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#5A3335] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#EAD9D5] p-6 space-y-4 max-w-2xl">
              <div className="flex items-center justify-between">
                <h3 className="font-['Playfair_Display'] text-lg text-[#292727]">{editProduct ? "Editar serviço" : "Novo serviço"}</h3>
                <button onClick={() => { setShowForm(false); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="text-[#697482] hover:text-[#292727]"><X size={18} /></button>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  {productImages.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={img} alt={`Imagem ${i + 1}`} className="w-20 h-20 rounded-xl object-cover border border-[#EAD9D5]" />
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#EAD9D5] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#697482] hover:border-[#7A4549] hover:text-[#292727] cursor-pointer transition-colors">
                ...
                <p className="text-[10px] text-[#697482]">{productImages.length}/5 imagens</p>
              </div>
              <div><label className={labelCls}>Nome</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="Ex: Tranças Naturais" /></div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {BELEZA_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {BELEZA_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#7A4549] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#5A3335] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {BELEZA_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {
          const groupProducts = getProductsForGroup(group.category);
          const isExpanded = selectedGroup === group.category;
          return (
            <div key={group.category} className="bg-white rounded-2xl border border-[#EAD9D5] overflow-hidden">
              <button onClick={() => setSelectedGroup(isExpanded ? null : group.category)} className="w-full flex items-center justify-between p-5 text-left hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-xs tracking-[0.2em] text-[#697482]">{group.number}</span>
                  <div>
                    <h4 className="text-sm font-semibold text-[#292727]">{group.title}</h4>
                    <p className="text-xs text-[#697482] mt-0.5">{groupProducts.length} serviço(s)</p>
                  </div>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-[#697482] transition-transform ${isExpanded ? "rotate-180" : ""}`}><path d="m6 9 6 6 6-6"/></svg>
              </button>
              {isExpanded && (
                <div className="border-t border-[#EAD9D5] p-5">
                  <p className="text-xs text-[#697482] mb-4">{group.intro}</p>
                  <div className="mb-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#697482] mb-2">Subcategorias</p>
                    <div className="flex flex-wrap gap-2">{group.items.map((item) => <span key={item} className="px-3 py-1.5 bg-[#F5F0EB] text-xs text-[#697482] rounded-full">{item}</span>)}</div>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#697482] mb-2">Serviços desta categoria</p>
                    {groupProducts.length === 0 ? (
                      <p className="text-xs text-[#697482] text-center py-6 bg-[#fafafa] rounded-xl">Nenhum serviço nesta categoria.</p>
                    ) : (
                      <div className="space-y-2">
                        {groupProducts.map((p: any) => (
                          <div key={p.id} className="flex items-center gap-3 p-3 bg-[#fafafa] rounded-xl">
                            ...
                            <div className="flex-1">
                              <h5 className="text-xs font-medium text-[#292727]">{p.name}</h5>
                              <p className="text-[10px] text-[#697482]">{p.subcategory || p.category} {p.price ? `· ${p.currency === "USD" ? "$" : p.currency === "EUR" ? "€" : p.currency === "GBP" ? "£" : "Kz"} ${p.price.toLocaleString("pt-AO")}` : ""}</p>
                            </div>
                            <button onClick={() => { setEditProduct(p); setForm({ name: p.name, price: String(p.price || ""), currency: p.currency || "AOA", category: p.category || "", subcategory: p.subcategory || "", description: p.description || "" }); setProductImages(p.imageUrls && p.imageUrls.length > 0 ? p.imageUrls : (p.imageUrl ? [p.imageUrl] : [])); setShowForm(true); }} className="text-[#697482] hover:text-[#7A4549] transition-colors p-1"><Edit2 size={13} /></button>
                            <button onClick={() => { if (confirm("Eliminar este serviço?")) deleteMut.mutate(p.id); }} className="text-[#697482] hover:text-red-500 transition-colors p-1"><Trash2 size={13} /></button>
                          </div>
                        ))}
                      </div>
                    )}
...
function ContactosSection({ store }: { store: any }) {
  return (
    <div>
      <h2 className="font-['Playfair_Display'] text-3xl text-[#292727] mb-8">Contactos</h2>
      <div className="bg-white rounded-2xl border border-[#EAD9D5] p-8 max-w-2xl space-y-6">
        <div><label className={labelCls}>WhatsApp</label><a href={`https://wa.me/244${store.whatsapp || store.phone}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-[#7A4549] hover:underline"><MessageCircle size={16} /> {store.whatsapp || store.phone}</a></div>
        <div><label className={labelCls}>Telefone</label><p className="text-sm text-[#292727]">{store.phone}</p></div>
        <div><label className={labelCls}>Endereço</label><p className="text-sm text-[#292727]">{store.address || "Não definido"}</p></div>
        <div>

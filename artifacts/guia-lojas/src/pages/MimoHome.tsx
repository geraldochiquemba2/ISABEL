      ...
      <nav className="mimo-nav" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, background: "rgba(248,241,231,0.9)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" }}>
        <div className="mimo-logo" style={{ textDecoration: "none", color: "inherit" }}>
          <img
            ...
            alt="YESOLA"
            style={{ width: "36px", height: "36px", filter: "brightness(0) saturate(100%) invert(42%) sepia(32%) saturate(1200%) hue-rotate(325deg) brightness(90%) contrast(90%)" }}
          />
        ...
        <div style={{ padding: "80px 16px 4px", maxWidth: "1380px", margin: "0 auto" }}>
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" as any }}>
            {services.map((service, i) => (
              <a
                key={service.title}
                href={`/explorar-love?servico=${encodeURIComponent(service.title)}`}
                onClick={(e) => { e.preventDefault(); window.location.href = `/explorar-love?servico=${encodeURIComponent(service.title)}`; }}

...
const EMPREGOS_CATEGORIES = [
  ...
  { number: "04", title: "Consultoria RH", intro: "Assessoria em gestão de recursos humanos.", items: ["Consultoria de RH", "Gestão de Pessoal", "Avaliação de Desempenho", "Plano de Carreira"], category: "consultoria" },
  { number: "05", title: "Freelancing & Freelance", intro: "Plataformas e serviços de freelancers.", items: ["Design Freelance", "Desenvolvimento Freelance", "Redação & Tradução", "Consultoria Freelance"], category: "freelancing" },
  { number: "06", title: "Empreendedorismo", intro: "Apoio a novos empreendedores e startups.", items: ["Incubadoras", "Aceleradoras", "Mentoria", "Financiamento"], category: "empreendedorismo" },
  { number: "07", title: "Área Jurídica & Trabalhista", intro: "Assessoria jurídica para questões trabalhistas.", items: ["Advogados Trabalhistas", "Mediação", "Contratos de Trabalho", "Direitos do Trabalhador"], category: "juridica" },
  { number: "08", title: "Educação & Ensino", intro: "Oportunidades na área de educação.", items: ["Professores", "Tutores", "Formadores", "Ensino Superior"], category: "educacao" },
  { number: "09", title: "Saúde & Farmácia", intro: "Vagas na área da saúde.", items: ["Enfermeiros", "Médicos", "Farmacêuticos", "Técnicos de Saúde"], category: "saude-emp" },
  { number: "10", title: "Tecnologia & Informática", intro: "Oportunidades no sector tecnológico.", items: ["Programadores", "Analistas de Sistemas", "Gestores de Projetos", "Suporte Técnico"], category: "tecnologia-emp" },
  { number: "11", title: "Comércio & Vendas", intro: "Vagas em comércio e vendas.", items: ["Vendedores", "Comerciantes", "Gestores de Loja", "Representantes Comerciais"], category: "comercio" },
  { number: "12", title: "Logística & Transportes", intro: "Oportunidades em logística e transportes.", items: ["Motoristas", "Armazéns", "Logística", "Distribuição"], category: "logistica" },
];

export default function DashboardEmpregos() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-empregos");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f5f3fa] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#4527A0] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#311B92] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-empregos")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f5f3fa]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f5f3fa] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#4527A0] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#B39DDB", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Empregos</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#4527A0] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#B39DDB", marginTop: "1px" }}>Empregos</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="empregos-oportunidades" accentColor="#4527A0" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#4527A0] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#311B92] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#4527A0] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#311B92] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d4e8d4] rounded-xl text-xs text-[#6B7280] hover:border-[#4527A0] hover:text-[#1a3a1a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#4527A0] hover:text-[#1a3a1a] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#4527A0" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#4527A0" />
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = EMPREGOS_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditProduct(null); setForm({ name: "", price: "", currency: "AOA", category: "", subcategory: "", description: "" }); setProductImages([]); }} className="flex items-center gap-2 bg-[#4527A0] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#311B92] transition-colors"><Plus size={15} /> Novo serviço</button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#4527A0] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {EMPREGOS_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {EMPREGOS_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#4527A0] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#311B92] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {EMPREGOS_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

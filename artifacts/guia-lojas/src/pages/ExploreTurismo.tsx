...
const TURISMO_CATEGORIES = [
  ...
  { number: "04", title: "Parques & Espaços de Lazer", intro: "Parques; espaços recreativos; parques infantis; centros de lazer.", category: "parques-lazer", icon: TreePine, items: ["Parques & Jardins", "Espaços Recreativos", "Parques Infantis", "Centros de Lazer"] },
  { number: "05", title: "Actividades Recreativas", intro: "Actividades ao ar livre; aventura; jogos; entretenimento recreativo.", category: "actividades-recreativas", icon: Gamepad2, items: ["Actividades ao Ar Livre", "Aventura & Desportos", "Jogos & Diversão", "Entretenimento Recreativo"] },
  { number: "06", title: "Turismo Cultural", intro: "Museus; monumentos; locais históricos; cultura; património.", category: "turismo-cultural", icon: Landmark, items: ["Museus & Galerias", "Monumentos & Locais Históricos", "Património Cultural", "Roteiros Culturais"] },
  { number: "07", title: "Guias Turísticos", intro: "Guias locais; culturais; acompanhamento turístico.", category: "guias-turisticos", icon: Users, items: ["Guias Locais", "Guias Culturais", "Acompanhamento Turístico", "Roteiros Personalizados"] },
];

function StoreCard({ store, productImages }: { store: any; productImages?: string[] }) {
  const fallbackImage = "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=400&h=300&fit=crop&auto=format&q=75";
  ...
  return (
    <div
      className="flex-shrink-0 w-48 rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-lg transition-shadow border border-[#B2DFDB] cursor-pointer hover:-translate-y-1"
      onClick={() => window.location.href = `/loja/${store.id}?from=turismo`}
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-[#0A2520] truncate">{store.name}</h4>
        {store.description && (
          <p className="text-[10px] text-[#4A6B65] mt-1 line-clamp-2">{store.description}</p>
        )}
      </div>
    </div>
  );
...
export default function ExploreTurismo() {
  const [activeFilter, setActiveFilter] = useState<string | null>(() => {
    ...
    if (cat) {
      const group = TURISMO_CATEGORIES.find((g) => g.title.toLowerCase().includes(cat.toLowerCase()) || g.category.toLowerCase() === cat.toLowerCase());
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products", "turismo-lazer"],
    queryFn: async () => {
      const res = await fetch("/api/products?store_type=turismo-lazer");
      if (!res.ok) return [];
      return res.json();
    },
    staleTime: 60_000,
  ...
  const getStoresForGroup = (category: string) => {
    const group = TURISMO_CATEGORIES.find((g) => g.category === category);
  ...
  const filteredGroups = activeFilter
    ? TURISMO_CATEGORIES.filter((g) => g.title.toLowerCase().includes(activeFilter.toLowerCase()) || g.category === activeFilter)
    : TURISMO_CATEGORIES;

  return (
    <main className="min-h-[100dvh] bg-[#F0FAF8] text-[#0A2520]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#F0FAF8]/95 backdrop-blur-md border-b border-[#00796B]/10">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-6 py-4 md:px-12">
          <button onClick={() => window.history.back()} className="flex items-center gap-2 text-sm text-[#68727c] hover:text-[#00796B] transition-colors">
            <ArrowLeft size={16} /> Voltar
          </button>
          <span className="font-['Playfair_Display'] text-[19px] tracking-[-.02em] text-[#00796B]">YESOLA<small className="block font-['DM_Sans'] text-[8px] uppercase tracking-[.23em] text-[#004D40] mt-0.5">Turismo & Lazer</small></span>
          <a href="/explorar-turismo" className="text-xs font-bold uppercase tracking-[0.14em] text-[#68727c] hover:text-[#00796B] transition-colors hidden md:block">Explorar</a>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1380px] px-6 pt-28 pb-12 md:px-12">
        ...
        <div className="mb-16">
          <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#00796B]">Explorar serviços</p>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          ...
          <button onClick={() => setActiveFilter(null)}
            className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
              activeFilter === null ? "bg-[#00796B] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
            }`}>Todos</button>
          {TURISMO_CATEGORIES.map((group) => (
            <button key={group.category} onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeFilter === group.category ? "bg-[#00796B] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
              }`}>{group.number} {group.title.split(",")[0].split(" e ")[0]}</button>
          ))}
        </div>

        <div className="mb-6">
          <span className="text-xs uppercase tracking-[0.15em] text-[#4A6B65] mr-2">Onde procuras?</span>
          <div className="mt-2 max-w-md">
            <WhereSearch onScope={setLocationScope} onClear={() => setLocationScope(null)} accent="#00796B" />
          </div>
          {locationScope && (
            <button
        ...
        <div className="mb-6">
          <span className="text-xs uppercase tracking-[0.15em] text-[#4A6B65] mr-2">Província:</span>
          <select
            value={activeProvince || ""}
            onChange={(e) => { setActiveProvince(e.target.value || null); setActiveMunicipality(null); }}
            className="mt-2 md:hidden w-full px-4 py-3 rounded-xl text-sm border border-[#B2DFDB] bg-white text-[#0A2520] outline-none"
          >
          ...
          <div className="hidden md:flex flex-wrap gap-3 mt-2">
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                activeProvince === null ? "bg-[#004D40] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
              }`}>Todas</button>
            {provinces.map((province) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeProvince === province ? "bg-[#004D40] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
                }`}>{province}</button>
            ))}
          </div>
        </div>
        ...
        {municipalities.length > 0 && (
          <div className="mb-12">
            <span className="text-xs uppercase tracking-[0.15em] text-[#4A6B65] mr-2">Município:</span>
            <select
            ...
            <div className="hidden md:flex flex-wrap gap-3 mt-2">
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                  activeMunicipality === null ? "bg-[#4A6B65] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
                }`}>Todos</button>
              {municipalities.map((m) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs uppercase tracking-[0.15em] transition-all ${
                    activeMunicipality === m ? "bg-[#4A6B65] text-white" : "bg-[#E0F2F1] text-[#68727c] hover:bg-[#B2DFDB]"
                  }`}>{m}</button>
              ))}
            </div>
          </div>
        ...
        <div>
          {filteredGroups.map((group, i) => {
            ...
            return (
              <article key={group.number} className={`group border-t border-[#00796B]/20 py-8 md:py-12 ${i % 2 ? "md:ml-20" : ""}`}>
                <div className="grid gap-7 md:grid-cols-[100px_minmax(0,1fr)_minmax(260px,370px)] md:items-start">
                  <span className="font-['DM_Sans'] text-xs font-bold tracking-[0.2em] text-[#00796B]">{group.number}</span>
                  <div>
                    <h3 className="max-w-xl font-['Playfair_Display'] text-3xl leading-[1.08] text-[#0A2520] md:text-[2.8rem]">{group.title}</h3>
                    <p className="mt-4 max-w-md text-sm leading-7 text-[#4A6B65]">{group.intro}</p>
                    <ul className="mt-6 space-y-3 border-l border-[#00796B]/30 pl-5 text-sm leading-5 text-[#4A6B65]">
                      {group.items.map((item) => (
                        <li key={item}>
                          <div className="flex gap-3">
                            <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#00796B]" />
                            <span>{item}</span>
                          </div>
                        </li>
                      ))}
                    ...
                    {groupStores.length > 0 && (
                      <button onClick={() => setActiveFilter(activeFilter === group.category ? null : group.category)}
                        className="mt-6 flex items-center gap-2 text-xs uppercase tracking-[0.15em] text-[#68727c] hover:text-[#00796B] transition-colors">
                        Ver mais
                      </button>
                    )}
                  </div>
                  <div className="mt-4 md:mt-0">
                    <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.2em] text-[#4A6B65] mb-3">Lojas/Serviços disponíveis</p>
                    {groupStores.length > 0 ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-[#00796B]/25 p-6 text-center">
                        <p className="text-xs text-[#4A6B65]">Em breve novas lojas</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="relative overflow-hidden border-t border-[#00796B]/20 bg-[#00796B] px-6 py-24 text-[#F0FAF8] md:px-12 md:py-32">
        <div className="relative mx-auto max-w-[1380px] md:flex md:items-end md:justify-between">
          <div>
            <p className="font-['DM_Sans'] text-[10px] uppercase tracking-[0.25em] text-[#B2DFDB]">O primeiro passo</p>
          ...
          <div className="mt-10 md:mt-0 md:w-80">
            <p className="text-sm leading-6 text-[#F0FAF8]/60">Conte-nos o que procura. A nossa equipa responde com tempo, atenção e cuidado.</p>
            <div className="mt-7">
              <a href="https://wa.me/244922001778?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20a%20YESOLA%20Turismo%20%26%20Lazer." target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#004D40] text-white text-sm font-medium rounded-full hover:bg-[#003D33] transition-colors">
                Falar connosco
              </a>
            </div>
          </div>

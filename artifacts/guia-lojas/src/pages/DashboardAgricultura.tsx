...
const inputCls = "w-full border border-[#d4e8d4] bg-white py-3 px-4 text-sm text-[#1a3a1a] placeholder:text-[#6B7280] outline-none focus:border-[#2E7D32] focus:ring-2 focus:ring-[#2E7D32]/10 transition-all rounded-xl";
...
const AGRICULTURA_CATEGORIES = [
  { number: "01", title: "Culturas & Produção Vegetal", intro: "Cultivo e produção de produtos agrícolas.", items: ["Cereais", "Frutas", "Hortaliças", "Legumes"], category: "culturas" },
  { number: "02", title: "Pecuária & Criação de Animais", intro: "Criação e maneio de animais.", items: ["Bovinos", "Suínos", "Avicultura", "Caprinos"], category: "pecuaria" },
  { number: "03", title: "Máquinas & Equipamentos Agrícolas", intro: "Maquinário e equipamento para agricultura.", items: ["Tratores", "Máquinas de Colheita", "Irrigação", "Ferramentas Agrícolas"], category: "maquinas" },
  { number: "04", title: "Sementes & Fertilizantes", intro: "Insumos para produção agrícola.", items: ["Sementes", "Fertilizantes", "Pesticidas", "Herbicidas"], category: "sementes" },
  { number: "05", title: "Aquicultura & Piscicultura", intro: "Produção piscícola e aquática.", items: ["Piscicultura", "Maricultura", "Aquicultura", "Camarão"], category: "aquicultura" },
  { number: "06", title: "Agroindústria & Transformação", intro: "Transformação de produtos agrícolas.", items: ["Moagens", "Laticínios", "Curtumes", "Enlatados"], category: "agroindustria" },
  { number: "07", title: "Comercialização & Mercado", intro: "Comercialização de produtos agrícolas.", items: ["Feiras", "Cooperativas", "Exportação", "Mercado Local"], category: "comercializacao" },
  { number: "08", title: "Consultoria & Assessoria Rural", intro: "Assessoria técnica para agricultores.", items: ["Consultoria Agronómica", "Maneio de Solo", "Controlo de Pragas", "Gestão Rural"], category: "consultoria-rural" },
  { number: "09", title: "Agricultura Biológica", intro: "Produção orgânica e sustentável.", items: ["Agricultura Orgânica", "Certificação", "Produtos Biológicos", "Agroecologia"], category: "bio" },
  { number: "10", title: "Formação & Extensão Rural", intro: "Capacitação para agricultores.", items: ["Cursos de Agronomia", "Extensão Rural", "Workshops", "Visitas Técnicas"], category: "formacao-rural" },
  { number: "11", title: "Financiamento & Crédito Agrícola", intro: "Apoio financeiro para o sector.", items: ["Crédito Agrícola", "Subsídios", "Microfinanças", "Seguros Agrícolas"], category: "financiamento" },
  { number: "12", title: "Transporte & Logística", intro: "Transporte e armazenamento de produtos.", items: ["Transporte de Produtos", "Armazéns", "Cadeia de Frio", "Distribuição"], category: "transporte-agri" },
];
...
export default function DashboardAgricultura() {
  ...
  useEffect(() => {
    if (!localUser) setLoc("/login-agricultura");
  }, [localUser, setLoc]);
  ...
  if (!isAdmin && localUser.status === "PENDENTE") {
    return (
      <PageTransition>
        <div className="min-h-screen bg-[#f0f7f0] flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button onClick={handleRefreshStatus} className="w-full bg-[#2E7D32] text-white py-2.5 rounded-full text-xs font-semibold hover:bg-[#1B5E20] transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={13} /> Atualizar Status</button>
              <button onClick={() => setLoc("/login-agricultura")} className="w-full border border-[#d4e8d4] text-[#6B7280] hover:bg-[#f0f0f0] py-2.5 rounded-full text-xs font-semibold transition-colors flex items-center justify-center gap-1.5">Voltar ao Login</button>
            </div>
          </div>
        </div>
      </PageTransition>
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#f0f7f0]"><p className="text-sm text-[#6B7280]">Carregando...</p></div>;

  return (
    <PageTransition>
      <div className="min-h-screen bg-[#f0f7f0] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#2E7D32] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em", color: "#fff" }}>YESOLA<small style={{ display: "block", color: "#A5D6A7", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", marginTop: "1px" }}>Agricultura</small></span>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-64 bg-[#2E7D32] text-white h-screen p-4 flex flex-col overflow-y-auto transition-transform duration-300`}>
          <div className="flex items-center gap-3 mb-5">
            <div>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", letterSpacing: "-.02em" }}>YESOLA</span>
              <p style={{ fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: ".23em", fontSize: "7px", color: "#A5D6A7", marginTop: "1px" }}>Agricultura</p>
        ...
        <main className="flex-1 p-4 pt-16 md:p-8 md:pt-8 overflow-y-auto">
          ...
          {section === "admin" && <AdminPanel storeType="agricultura-agronegocio" accentColor="#2E7D32" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-8 max-w-sm w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleForceChangePwd} disabled={pwdLoading} className="flex-1 bg-[#2E7D32] text-white py-3 rounded-xl text-sm font-medium hover:bg-[#1B5E20] transition-colors">{pwdLoading ? "A guardar..." : "Guardar"}</button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {isDirty && <button onClick={handleSave} disabled={mutation.isPending} className="bg-[#2E7D32] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#1B5E20] transition-colors">{mutation.isPending ? "A guardar..." : "Guardar alterações"}</button>}
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#d4e8d4] rounded-xl text-xs text-[#6B7280] hover:border-[#2E7D32] hover:text-[#1a3a1a] cursor-pointer transition-colors">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-24 h-24 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#2E7D32] hover:text-[#1a3a1a] cursor-pointer transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#d4e8d4] p-8 space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <CategoryMultiSelect options={getAreaCategories("agricultura-agronegocio")} value={form.categories ?? []} onChange={(next) => { setForm((prev) => ({ ...prev, categories: next })); setDirty(true); }} max={4} accent="#2E7D32" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <LocationCombobox label="Província" value={form.province} options={ANGOLA_PROVINCES.map((p) => p.name)} onChange={(v) => { handleChange("province", v); handleChange("municipality", ""); handleChange("locality", ""); }} placeholder="Selecione a Província" accent="#2E7D32" />
          <LocationCombobox label="Município" value={form.municipality} options={ANGOLA_PROVINCES.find((p) => p.name === form.province)?.municipalities || []} onChange={(v) => { handleChange("municipality", v); handleChange("locality", ""); }} placeholder={form.province ? "Selecione o Município" : "Selecione a província primeiro"} disabled={!form.province} accent="#2E7D32" />
        </div>
        <LocationCombobox label="Localidade exacta" value={form.locality} options={getLocalities(form.province, form.municipality)} onChange={(v) => handleChange("locality", v)} placeholder={form.municipality ? "Selecione a localidade" : "Selecione o município primeiro"} disabled={!form.municipality} accent="#2E7D32" />
        <div>
...
function ProdutosSection({ store }: { store: any }) {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const selectedCat = AGRICULTURA_CATEGORIES.find((g) => g.title === form.category);
  ...
  return (
    <div>
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
            <div className="bg-white rounded-2xl border border-[#d4e8d4] p-6 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mb-3">
                  ...
                  {productImages.length < 5 && (
                    <label className="w-20 h-20 border border-dashed border-[#d4e8d4] rounded-xl flex flex-col items-center justify-center text-[10px] text-[#6B7280] hover:border-[#2E7D32] hover:text-[#1a3a1a] cursor-pointer transition-colors">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className={inputCls}>
                    ...
                    {AGRICULTURA_CATEGORIES.map((group) => <option key={group.category} value={group.title}>{group.number} — {group.title.split(",")[0]}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <select value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })} className={inputCls} disabled={!form.category}>
                  <option value="">Selecionar...</option>
                  {AGRICULTURA_CATEGORIES.find((g) => g.title === form.category)?.items.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </div>
              ...
              <button onClick={() => createMut.mutate()} disabled={createMut.isPending || !form.name} className="bg-[#2E7D32] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-[#1B5E20] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-6">
        {AGRICULTURA_CATEGORIES.filter((group) => getProductsForGroup(group.category).length > 0).map((group) => {

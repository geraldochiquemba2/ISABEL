  ...
  return (
    <PageTransition>
      <div className="space-y-6 max-w-5xl">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#123c4a] flex items-center gap-2">
        ...
        <div className="flex border-b border-[#d1d4d8] gap-1">
          {([
            ...
            { id: "categorias" as MainTab, label: "Categorias de Formação", icon: <Tag size={13} /> },
          ]).map((t) => (
            <button
              key={t.id}
              ...
              className={`pb-3 px-5 text-xs font-semibold uppercase tracking-wider relative transition-colors flex items-center gap-1.5 ${
                mainTab === t.id
                  ? "text-[#123c4a] font-bold"
                  : "text-[#87909a] hover:text-[#123c4a]"
              }`}
            >
              {t.icon} {t.label}
              {mainTab === t.id && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#0c9894]" />}
            </button>
          ))}
        </div>
...
function ContasSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try { setUsers(await fetchAdminUsersFiltered("formacoes")); }
    ...
    <div className="space-y-4">
          ...
          return (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSearchTerm(""); }}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                activeTab === t.id ? "bg-[#0c9894] text-white border-[#0c9894]" : "border-[#d1d4d8] text-[#87909a] hover:border-[#0c9894] hover:text-[#123c4a]"
              }`}>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Pesquisar por nome, telefone, loja..."
          ...
          className="w-full border border-[#d1d4d8] rounded-2xl pl-9 pr-4 py-2.5 text-xs outline-none focus:ring-1 focus:ring-[#0c9894] bg-white" />
      </div>
      ...
      {rejectId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#0c9894]" />
      ...
      {suspendId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={suspendReason} onChange={(e) => setSuspendReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#0c9894]" />
            <div className="flex justify-end gap-2 text-xs">
...
function CategoriasSection() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[#87909a]">{groups.length} categorias de formação</p>
                ...
                {g.items?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {g.items.map((item: string) => (
                      <span key={item} className="text-[10px] bg-[#f0f7f7] text-[#123c4a] px-2 py-0.5 rounded-full border border-[#d1d4d8]">{item}</span>
                    ))}
                  </div>
                )}
              </div>
              <button onClick={() => startEdit(g)} className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full border border-[#d1d4d8] hover:bg-[#f0f7f7] transition-colors flex-shrink-0">
          ...
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input type="text" placeholder="Adicionar serviço..." value={editItemInput} onChange={(e) => setEditItemInput(e.target.value)}
                  ...
                  className="flex-1 border border-[#d1d4d8] rounded-xl px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#0c9894]" />
                <button onClick={handleAddItem} className="px-4 py-2 bg-[#f0f7f7] text-[#123c4a] font-semibold text-xs rounded-xl hover:bg-[#d1d4d8] transition-colors">Add</button>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {editItems.map((item) => (
                  <span key={item} className="inline-flex items-center gap-1 bg-[#f0f7f7] px-2.5 py-1 rounded-md text-xs font-medium border border-[#d1d4d8]">
            ...
            <div className="flex justify-end gap-2 text-xs">
              ...
              <button onClick={handleSave} disabled={!editTitle.trim() || saving}
                className="px-4 py-2 bg-[#0c9894] text-white rounded-full hover:bg-[#087c7c] transition-colors disabled:opacity-50 flex items-center gap-1.5">

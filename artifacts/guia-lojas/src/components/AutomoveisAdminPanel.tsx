...
import {
  ...
  KeyRound, Trash2, RefreshCw, Tag, Edit2
} from "lucide-react";
import {
  fetchAdminUsersFiltered, approveLojista, rejectLojista, suspendLojista, reactivateLojista, resetUserPassword,
  fetchWeddingGroups, updateWeddingGroup,
} from "@/lib/api";

type MainTab = "contas" | "categorias";
type AccountTab = "PENDENTE" | "APROVADO" | "DESATIVADO";

const inputCls = "w-full border border-[#d1d4d8] bg-white py-3 px-4 text-sm text-[#30343a] placeholder:text-[#87909a] outline-none focus:border-[#c9913a] focus:ring-2 focus:ring-[#c9913a]/10 transition-all rounded-xl";
const labelCls = "block text-[10px] font-semibold uppercase tracking-widest text-[#87909a] mb-1.5";

const AUTOMOVEIS_CATEGORIES = [
  { number: "01", title: "Venda de Carros Novos & Usados", category: "venda-carros" },
  { number: "02", title: "Aluguer de Viaturas", category: "aluguer-viaturas" },
  { number: "03", title: "Oficinas & Mecânicos", category: "oficinas-mecanicos" },
  { number: "04", title: "Peças & Acessórios", category: "pecas-acessorios" },
  { number: "05", title: "Lavagem & Detailing", category: "lavagem-detailing" },
  { number: "06", title: "Serviços de Transporte", category: "servicos-transporte" },
  { number: "07", title: "Assistência em Viagem", category: "assistencia-viagem" },
  { number: "08", title: "Seguros Automóvel", category: "seguros-auto" },
  { number: "09", title: "Inspecção & Documentação", category: "insepcao-doc" },
  { number: "10", title: "Estacionamentos & Garagens", category: "estacionamentos" },
];

export function AutomoveisAdminPanel() {
      ...
      <div className="space-y-6 max-w-5xl">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#30343a] flex items-center gap-2">
            <ShieldAlert size={24} /> Administração Automóveis
          </h1>
          <p className="text-sm text-[#87909a] mt-1">
            Gerencie contas e categorias da YESOLA Automóveis & Mobilidade
          </p>
        </div>
        ...
        <div className="flex border-b border-[#d1d4d8] gap-1">
          {([
            ...
            >
              {t.icon} {t.label}
              {mainTab === t.id && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#c9913a]" />}
            </button>
          ))}
        </div>
...
function ContasSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try { setUsers(await fetchAdminUsersFiltered("automoveis")); }
    ...
    <div className="space-y-4">
          ...
          return (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSearchTerm(""); }}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                activeTab === t.id ? "bg-[#c9913a] text-white border-[#c9913a]" : "border-[#d1d4d8] text-[#87909a] hover:border-[#c9913a] hover:text-[#30343a]"
              }`}>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Pesquisar por nome, telefone, loja..."
          ...
          className="w-full border border-[#d1d4d8] rounded-2xl pl-9 pr-4 py-2.5 text-xs outline-none focus:ring-1 focus:ring-[#c9913a] bg-white" />
      </div>
      ...
      {rejectId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#c9913a]" />
      ...
      {suspendId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={suspendReason} onChange={(e) => setSuspendReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#c9913a]" />
            <div className="flex justify-end gap-2 text-xs">
...
function CategoriasSection() {
  ...
  async function loadGroups() {
    setLoading(true);
    try { setGroups(await fetchWeddingGroups()); }
    catch (e) { console.error(e); }
    finally { setLoading(false); }
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await updateWeddingGroup(editId, {
        number: groups.find(g => g.id === editId)?.number || "00",
  ...
  if (loading) return <div className="text-center py-12 text-sm text-[#87909a]">A carregar categorias...</div>;

  return (
    <div className="space-y-4">
      <p className="text-sm text-[#87909a]">{groups.length} categorias de serviços automóvel</p>

      <div className="space-y-3">
        {groups.map((g) => (
          <div key={g.id} className="border border-[#d1d4d8] rounded-2xl p-4 bg-white shadow-sm">
            <div className="flex items-start gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-xs text-[#87909a] font-mono">#{g.number} · {g.category}</p>
                <p className="text-sm font-semibold mt-0.5">{g.title}</p>
                <p className="text-xs text-[#87909a] mt-1">{g.intro}</p>
                {g.items?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {g.items.map((item: string) => (
                      <span key={item} className="text-[10px] bg-[#e2e8f0] text-[#30343a] px-2 py-0.5 rounded-full border border-[#d1d4d8]">{item}</span>
                    ))}
                  </div>
                )}
              </div>
              <button onClick={() => startEdit(g)} className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full border border-[#d1d4d8] hover:bg-[#e2e8f0] transition-colors flex-shrink-0">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <textarea value={editIntro} onChange={(e) => setEditIntro(e.target.value)} rows={2} className={inputCls} />
            </div>
            ...
            <div>
              ...
              <select value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className={`${inputCls} cursor-pointer`}>
                {AUTOMOVEIS_CATEGORIES.map((cat) => (
                  <option key={cat.category} value={cat.category}>{cat.title}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input type="text" placeholder="Adicionar serviço..." value={editItemInput} onChange={(e) => setEditItemInput(e.target.value)}
                  ...
                  className="flex-1 border border-[#d1d4d8] rounded-xl px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#c9913a]" />
                <button onClick={handleAddItem} className="px-4 py-2 bg-[#e2e8f0] text-[#30343a] font-semibold text-xs rounded-xl hover:bg-[#d1d4d8] transition-colors">Add</button>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {editItems.map((item) => (
                  <span key={item} className="inline-flex items-center gap-1 bg-[#e2e8f0] px-2.5 py-1 rounded-md text-xs font-medium border border-[#d1d4d8]">
            ...
            <div className="flex justify-end gap-2 text-xs">
              ...
              <button onClick={handleSave} disabled={!editTitle.trim() || saving}
                className="px-4 py-2 bg-[#c9913a] text-white rounded-full hover:bg-[#b57e30] transition-colors disabled:opacity-50 flex items-center gap-1.5">

...
import {
  ...
  KeyRound, Plus, Trash2, RefreshCw, Tag, Edit2
} from "lucide-react";
import {
...
export function WeddingAdminPanel() {
      ...
      <div className="space-y-6 max-w-5xl">
        ...
        <div className="flex border-b border-[#d1d4d8] gap-1">
          {([
            ...
            { id: "categorias" as MainTab, label: "Categorias de Casamento", icon: <Tag size={13} /> },
          ]).map((t) => (
            <button
              key={t.id}
...
function ContasSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try { setUsers(await fetchAdminUsersFiltered("weddings")); }
    ...
    <div className="space-y-4">
          ...
          return (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSearchTerm(""); }}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                activeTab === t.id ? "bg-[#2c3035] text-white border-[#2c3035]" : "border-[#d1d4d8] text-[#87909a] hover:border-[#2c3035] hover:text-[#30343a]"
              }`}>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Pesquisar por nome, telefone, loja..."
          ...
          className="w-full border border-[#d1d4d8] rounded-2xl pl-9 pr-4 py-2.5 text-xs outline-none focus:ring-1 focus:ring-[#2c3035] bg-white" />
      </div>
      ...
      {rejectId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#2c3035]" />
      ...
      {suspendId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={suspendReason} onChange={(e) => setSuspendReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#2c3035]" />
            <div className="flex justify-end gap-2 text-xs">
...
function CategoriasSection() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[#87909a]">{groups.length} categorias de casamento</p>
          ...
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input type="text" placeholder="Adicionar serviço..." value={editItemInput} onChange={(e) => setEditItemInput(e.target.value)}
                  ...
                  className="flex-1 border border-[#d1d4d8] rounded-xl px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#2c3035]" />
            ...
            <div className="flex justify-end gap-2 text-xs">
              ...
              <button onClick={handleSave} disabled={!editTitle.trim() || saving}
                className="px-4 py-2 bg-[#2c3035] text-white rounded-full hover:bg-[#1a1d20] transition-colors disabled:opacity-50 flex items-center gap-1.5">

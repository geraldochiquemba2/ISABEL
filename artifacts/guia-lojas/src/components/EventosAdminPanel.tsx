...
const EVENTOS_CATEGORIES = [
  { number: "01", title: "Planeamento, Design e Assessoria de Eventos", category: "decoracao" },
  { number: "02", title: "Estrutura, Mobilia e Aluguer de Equipamentos", category: "equipamentos" },
  { number: "03", title: "Gastronomia, Bar e Restauração para Eventos", category: "gastronomia" },
  { number: "04", title: "Animação, Entretenimento e Atração de Pistas", category: "animacao" },
  { number: "05", title: "Staff Profissional e Segurança", category: "staff" },
  { number: "06", title: "Registo, Memória e Fotografia", category: "fotografia" },
];
  ...
  return (
    <PageTransition>
      <div className="space-y-6 max-w-5xl">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#3c2731] flex items-center gap-2">
        ...
        <div className="flex border-b border-[#d1d4d8] gap-1">
          {([
            ...
            { id: "categorias" as MainTab, label: "Categorias de Eventos", icon: <Tag size={13} /> },
          ]).map((t) => (
            <button
              key={t.id}
              ...
              className={`pb-3 px-5 text-xs font-semibold uppercase tracking-wider relative transition-colors flex items-center gap-1.5 ${
                mainTab === t.id
                  ? "text-[#3c2731] font-bold"
                  : "text-[#87909a] hover:text-[#3c2731]"
              }`}
            >
              {t.icon} {t.label}
              {mainTab === t.id && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#8e5557]" />}
            </button>
          ))}
        </div>
...
function ContasSection() {
  ...
  async function loadUsers() {
    setLoading(true);
    try { setUsers(await fetchAdminUsersFiltered("eventos")); }
    ...
    <div className="space-y-4">
          ...
          return (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSearchTerm(""); }}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                activeTab === t.id ? "bg-[#8e5557] text-white border-[#8e5557]" : "border-[#d1d4d8] text-[#87909a] hover:border-[#8e5557] hover:text-[#3c2731]"
              }`}>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Pesquisar por nome, telefone, loja..."
          ...
          className="w-full border border-[#d1d4d8] rounded-2xl pl-9 pr-4 py-2.5 text-xs outline-none focus:ring-1 focus:ring-[#8e5557] bg-white" />
      </div>
        ...
        {filtered.map((user) => (
          <div key={user.id} className="border border-[#d1d4d8] rounded-2xl p-4 bg-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div className="flex items-start gap-3 flex-1">
              <div className="w-14 h-14 rounded-full overflow-hidden bg-[#f0e6df] flex-shrink-0">
      ...
      {rejectId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#8e5557]" />
      ...
      {suspendId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <textarea placeholder="Motivo detalhado..." value={suspendReason} onChange={(e) => setSuspendReason(e.target.value)} rows={4} className="w-full border border-[#d1d4d8] rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-[#8e5557]" />
            <div className="flex justify-end gap-2 text-xs">
...
function CategoriasSection() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[#87909a]">{groups.length} categorias de eventos</p>
                ...
                {g.items?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {g.items.map((item: string) => (
                      <span key={item} className="text-[10px] bg-[#f0e6df] text-[#3c2731] px-2 py-0.5 rounded-full border border-[#d1d4d8]">{item}</span>
                    ))}
                  </div>
                )}
              </div>
              <button onClick={() => startEdit(g)} className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full border border-[#d1d4d8] hover:bg-[#f0e6df] transition-colors flex-shrink-0">
          ...
          <div className="bg-white rounded-2xl border border-[#d1d4d8] shadow-xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input type="text" placeholder="Adicionar serviço..." value={editItemInput} onChange={(e) => setEditItemInput(e.target.value)}
                  ...
                  className="flex-1 border border-[#d1d4d8] rounded-xl px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#8e5557]" />
                <button onClick={handleAddItem} className="px-4 py-2 bg-[#f0e6df] text-[#3c2731] font-semibold text-xs rounded-xl hover:bg-[#d1d4d8] transition-colors">Add</button>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {editItems.map((item) => (
                  <span key={item} className="inline-flex items-center gap-1 bg-[#f0e6df] px-2.5 py-1 rounded-md text-xs font-medium border border-[#d1d4d8]">
            ...
            <div className="flex justify-end gap-2 text-xs">
              ...
              <button onClick={handleSave} disabled={!editTitle.trim() || saving}
                className="px-4 py-2 bg-[#8e5557] text-white rounded-full hover:bg-[#7a4849] transition-colors disabled:opacity-50 flex items-center gap-1.5">

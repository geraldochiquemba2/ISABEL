...
styleTipsRouter.get("/", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM style_tips ORDER BY ordem ASC, id ASC"
    );
...
styleTipsRouter.post("/", async (req, res) => {
  try {
    const { titulo, descricao, imagem, dicas, ordem } = req.body;
    const dicasArr = Array.isArray(dicas) ? dicas : [];
    const result = await pool.query(
      `INSERT INTO style_tips (titulo, descricao, imagem, dicas, ordem) 
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [titulo, descricao, imagem, dicasArr, ordem || 0]
    );
...
styleTipsRouter.put("/:id", async (req, res) => {
  try {
    ...
    const dicasArr = Array.isArray(dicas) ? dicas : [];
    await pool.query(
      `UPDATE style_tips SET titulo=$2, descricao=$3, imagem=$4, dicas=$5, ordem=$6 WHERE id=$1`,
      [id, titulo, descricao, imagem, dicasArr, ordem || 0]
    );
